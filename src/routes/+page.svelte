<script lang="ts">
	...
	// Das Onboarding bleibt auch nach dem Verknüpfen stehen, solange es noch
	...
	// ein Geräte-Token, das ueber "Entkoppeln" gelöst wird.
	//
	// Ohne Rückfrage: die Sitzung ist mit einem Passkey in Sekunden zurueck.
	// AUSSER es gibt keinen - dann ist die Abmeldung eine Einbahnstrasse, und
	// zurueck käme man nur ueber die 24 Wörter.
	let withoutPasskey = $state(false);
	let creating = $state(false);
	/** Läuft gerade eine Nachfrage oder das Abmelden? Solange dreht sich der Knopf. */
	...
	async function logout() {
		...
		// die App danach unverändert da, und niemand weiss, ob etwas passiert.
		busy = true;
		try {
	...
	async function createPasskey() {
		...
		try {
			...
			if (!prfAvailable) {
				...
				// Abgemeldet bräuchte es trotzdem die 24 Wörter - also bleibt der
				// Hinweis stehen, statt ein Versprechen zu geben, das nicht hält.
	...
	// Läuft nur, solange auf ein sichtbares Fenster gewartet wird (siehe unten).
	...
	/** Warten, bis das Hauptfenster tatsächlich zu sehen ist. */
	function whenWindowVisible(): Promise<void> {
		...
		visibilityWait ??= new Promise((resolve) => {
			const done = () => {
				...
				// Vor dem Auflösen: ein späterer Aufruf soll neu nachsehen, das
				// Fenster kann dann längst wieder weg sein.
				visibilityWait = null;
				resolve();
			};
			const check = async () => {
	...
	async function checkAndAnnounceUpdate() {
		...
		// zöge das dem Dialog das Update unter den Füssen weg.
		...
		// Vor dem Warten merken: sonst stellte sich stündlich ein weiterer Hinweis
		// zu derselben Version an, solange das Fenster versteckt ist – und beim
		// Öffnen kämen sie alle auf einmal.
		...
		// den veralteten Hinweis verwerfen. Den Vermerk dabei zurücknehmen - sonst
		// bliebe genau diese Version für den Rest des Laufs stumm, obwohl der
	...
	async function startup() {
		...
		// Nicht abwarten: das Aufräumen alter Tage darf den Start nicht bremsen.
		void pruneOldLogs();
		...
		try {
			...
			// Alles Weitere gibt es nur in der Desktop-Hülle: Tray-Ereignisse,
			// globale Hotkeys, Leerlauf-Erkennung, Autostart und die Update-Suche.
			// Im Browser würde jeder dieser Aufrufe werfen - und zwar mitten in der
			...
			if (unlisteners.length === 0) {
				unlisteners.push(
					...
					// Kein Tabwechsel und kein Toast mehr: PairApprovalDialog geht damit
					// von selbst auf, egal wo jemand gerade ist.
					await onPairLink(
						(code) => {
							account.pairCodeFromLink = code;
						},
						// Der Browser bittet diese Anwendung, selbst zu koppeln.
						(server) => {
			...
			// Autostart bei Login standardmässig aktivieren (läuft dann versteckt im Tray).
	...
	/**
	 * Zweiter Klick statt Dialog: der Fehlerbildschirm läuft, wenn die Anwendung
	 * sonst nichts kann - je weniger daran hängt, desto besser.
	 */
	 ...
	 * früher, wenn der Abgleich schneller fertig ist. Ohne Netz läuft `syncNow`
	...
	// Sekunden im Ladebildschirm. Ab einer Weile ist "Lädt…" keine Auskunft mehr,
	// sondern nur noch die Frage, ob überhaupt etwas passiert – dann nennt der
	// Bildschirm den Schritt, an dem es hängt.
	let waiting = $state(0);
	$effect(() => {
	...
	//
	// Am Verschwinden der Anmeldeseite, NICHT am Verknüpfen: gekoppelt wird aus
...
{#if !app.loaded || (!isTauri() && !account.ready)}
	<!-- Ladebildschirm mit Auskunft: welcher Schritt läuft, wie lange schon, und
		...
		<div class="w-full max-w-md space-y-4 text-center">
			...
			{#if app.initError}
				...
				<!-- Hilft "Erneut versuchen" nicht, gäbe es im Browser sonst keinen Weg
				     ...
				     Rechner nicht angeboten - dort würde es die Zeiten des Menschen
				     löschen, und der Datenordner steht ihm ohnehin offen. -->
				{#if !isTauri()}
					{#if confirmReset}
			...
			{:else}
				<!-- Im Browser bleibt es still: dort dauert der Start Millisekunden, und
				     ...
				     hängt - dort steht sie sofort. Zieht es sich hier doch, sagt der
				...
				<div class="col-start-1 row-start-1 flex items-center gap-2 justify-self-start">
					...
					{#if isBeta}
						<!-- Neben dem Logo, nicht rechts bei den Statusanzeigen: das ist keine
						     Meldung, die kommt und geht, sondern gilt für diese Installation. -->
...
<!--
	Der wichtigste Zeitpunkt für einen Abgleich: das Fenster kommt zurueck. Wer
