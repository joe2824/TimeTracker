...
/** Was sich nie ändert: die Dateinamen tragen die Fassung. */
...
sw.addEventListener("install", (event) => {
	event.waitUntil(
		caches
			...
			// Sofort übernehmen: sonst bliebe nach einem Update die alte Fassung
...
async function answer(req: Request, url: URL): Promise<Response> {
	...
	// Das gebaute Programm hat die Fassung im Namen und ändert sich nie: aus dem
	// Vorrat, ohne zu fragen. Der Rahmen gehört ausdrücklich NICHT dazu - er
	// liegt unter einer festen Adresse und muss sich erneuern dürfen.
	...
	try {
		...
		// Nur echte Antworten aufheben. Eine Fehlerseite im Vorrat wäre schlimmer
		// als gar keine.
