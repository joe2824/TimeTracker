...
beforeEach(async () => {
	resetFakeFs();
	await flushLog(); // Reste aus dem vorigen Test nicht in den nächsten tragen
	files.clear();
});
...
describe("Protokollieren", () => {
	...
	it("schreibt Fehler sofort – ohne sie im Puffer zu sammeln", async () => {
		...
		// Fehler wäre genau die, die fehlt.
	...
	it("nimmt die Aufrufliste eines Fehlers mit – eingerueckt als Fortsetzung", async () => {
		...
		// Fortsetzungszeilen beginnen mit Tab: sonst sähen sie wie eigene Einträge
	...
	it("bleibt still, wenn das Dateisystem nicht mitspielt", async () => {
		...
		// Dateisystem klemmt, über das sich der Fehler melden liesse.
