...
/** Frischer App-Zustand ohne init() – das würde den Sekunden-Tick starten. */
function reset(entries: Record<string, Entry[]> = {}) {
	resetFakeFs();
	// Ein KONFIGURIERTES Konto nachstellen: ohne gespeicherte Zeitzone übernähme
	// reload() die des Geräts und die Suite prüft dann nur noch sich selbst.
	files.set(
		"data/settings.json",
...
describe("stop() – Teilung an Mitternacht", () => {
	...
	it("legt das Folgetag-Stück in die richtige MONATSDATEI", async () => {
		// Über die Monatsgrenze: sonst zählte die Zeit im falschen Bericht.
...
describe("stop() – Lauf über mehrere Tage nachträglich beenden", () => {
	 ...
	 * zugeklappt. Der Mitternachts-Wechsel hat daraus vier Stücke gemacht (zwei
	 * davon volle 24-Stunden-Tage). Wer im "Timer läuft noch"-Dialog das echte
	 * Ende einträgt, meint den ganzen Lauf – nicht nur das letzte Stück.
	 */
	...
	it("kürzt das Stück, in dem die Endzeit liegt, und löscht nur die danach", async () => {
		...
		expect(es[2].endTs).toBe(at(18, 10)); // t3 gekürzt, t4 weg
	});

	it("lässt einen Lauf nie spurlos verschwinden", async () => {
		runsOverThreeDays();

		await app.stop(at(15, 8)); // vor dem Start – unmöglich
	...
	/**
	 * Zwei offene Stücke desselben Laufs, getrennt an Mitternacht - das
	 * Folgestück läuft.
	 */
	...
	it("räumt das Folgestück auch dann weg, wenn beide Stücke offen stehen", async () => {
		// Aus dem Protokoll: die Mitternachts-Teilung hat das Vorgängerstück offen
		// stehen lassen, damit fehlte das Bindeglied `endTs`. Das Folgestück galt
		// als eigener Lauf, dessen Beginn schon hinter der Endzeit lag – und blieb
		// als Eintrag über 00:00–00:00 zurück.
		twoOpenPieces();
	...
	it("erkennt den Lauf über ein offenes Vorgängerstück hinweg", async () => {
		// Dieselbe Lage, aber die Endzeit liegt im Folgestück: dann bleiben beide
	...
	it("fasst zwei zufällig aneinanderstoßende Einträge nicht zu einem Lauf zusammen", async () => {
		// Manuell erfasst, kein Mitternachts-Stück: der Vormittag bleibt stehen.
...
describe("Rückdatierter Start kürzt einen Eintrag über mehrere Tage", () => {
	it("teilt den gekürzten Eintrag an Mitternacht, statt eine Mehrtages-Zeile zu lassen", async () => {
		...
		// Start gekürzt – ohne Teilung stünde dort ein Eintrag über 49 Stunden.
...
describe("Zeitausgleich", () => {
	it("legt ihn auf der Abwesenheits-Zeile an und merkt sich die Art", async () => {
		// Keine eigene Aktivität: der Unterschied steckt allein im Eintrag.
		reset();
	...
	it("schreibt an einen gewoehnlichen Urlaubstag gar kein Merkmal", async () => {
		// Ein `timeOff: false` an jedem Eintrag wäre eine inhaltliche Änderung
		// und schickte beim nächsten Abgleich den halben Bestand erneut hoch.
		reset();
...
describe("deleteYearEntries", () => {
	it("löscht nur das genannte Jahr – aus Datei UND Cache", async () => {
		...
		expect(app.entriesByMonth["2025-12"]).toBeUndefined(); // Cache mit geräumt
		expect(onDisk("2026-07")).toHaveLength(1);
	});
...
describe("Zurückgebliebene offene Einträge (Absturz)", () => {
	...
	it("kappt am eigenen Tagesende, statt ein Wochenende zu schlucken", async () => {
		// Absturz am Freitag, App erst Montag wieder auf. Ohne Kappung stünden
		// hier 72 Stunden.
...
describe("Teilung respektiert Ganztags-Abwesenheiten", () => {
	it("legt kein Folgetag-Stück an, wenn der neue Tag ganztags abwesend ist", async () => {
		...
		// überall sonst durchsetzt: Urlaubstag trägt keine Projektzeit.
		const vacation: Entry = { ...entry("v", ABS, at(17, 12), at(17, 12)), dayFraction: 1 };
		...
		expect(project[0].endTs).toBe(at(17, 0)); // endet an Mitternacht, kein Stück danach
	});
	...
	it("addEntry (ohne Rueckfrage-Option) lehnt einen Folgetag mit Ganztags-Abwesenheit ab", async () => {
		...
		expect(app.absenceOverridePrompt).toBeNull(); // keine Rückfrage ohne Opt-in
...
describe("Rueckfrage bei Ganztags-Abwesenheit auf einem Folgetag", () => {
	it("addEntry mit confirmAbsenceOverride zeigt eine Rueckfrage statt zu schreiben", async () => {
		...
		expect(result).toBeNull();
		expect(onDisk("2026-07")).toHaveLength(1); // noch nichts verändert
	...
	it("updateEntry zeigt dieselbe Rueckfrage, ohne den Eintrag vorher zu veraendern", async () => {
		...
		expect(disk.endTs).toBe(at(17, 23)); // unverändert, solange die Rückfrage offen ist
	});
	...
	it("confirmAbsenceOverride entfernt die Abwesenheit und uebernimmt die Bearbeitung (update)", async () => {
		...
		expect(e.endTs).toBe(at(18, 0)); // Tag 1 behält die id, endet an Mitternacht
...
describe("Bearbeiten eines anstossenden Timer-Eintrags", () => {
	...
	// Timer-Einträge stossen aber sekundengenau aneinander: ein Aktivitätswechsel
	// setzt das Ende des einen exakt auf den Start des nächsten.
	...
	it("nimmt den Start mit erhaltenen Sekunden an", async () => {
		...
		const ok = await app.updateEntry(afterwards.startTs, {
			...afterwards,
			startTs: sec(19, 14, 0, 22), // keepSeconds hat die 22 zurückgegeben
			endTs: sec(19, 18, 26, 0)
		});
		expect(ok).toBe(true);
...
describe("#reportConflict über Monatsgrenzen", () => {
	it("erkennt eine Überschneidung, wenn der Kandidat über den Monatswechsel reicht", async () => {
		// #reportConflict muss auch den Folgemonat prüfen: ein Kandidat, der über
		// den Monatswechsel reicht, darf einen Eintrag am 1. August nicht übersehen.
...
describe("init() – was der Ladebildschirm anzeigt", () => {
	...
	// geladen" sein, sonst hängt der Sekunden-Tick in der Suite weiter.
	afterEach(() => {
		app.dispose();
		app.loaded = false;
		app.initStep = null;
...
describe("devSimulateStartFault() – Ladebildschirm vorführen", () => {
	...
	it("nimmt den echten Fehlerweg und lässt sich danach normal starten", async () => {
		...
		reset({ "2026-07": [ongoing] });
		app.loaded = true; // App läuft bereits
...
describe("ensureMonth", () => {
	it("liest eine Datei auch bei gleichzeitigen Anfragen nur einmal", async () => {
		...
		// Zwölfer-Blöcken (Auswertung, Arbeitszeit-Check, Tracking-Hinweis).
		// `ensureMonth` prüft VOR dem await, ob der Monat da ist – ohne
		// Merkliste käme jede Anfrage durch, und dieselbe Datei würde vierfach
		// gelesen.
		reset();
	...
	it("laedt nach einem fehlgeschlagenen Versuch erneut", async () => {
		// Die Merkliste darf den Monat nicht dauerhaft blockieren: schlägt das
		// Lesen fehl, muss der nächste Aufruf es wieder versuchen dürfen.
		reset();
...
describe("Eingebaute Zeilen: Duplikate zusammenfuehren", () => {
	/** Aktivitäten auf die Platte legen und die App neu einlesen lassen. */
	...
	it("macht aus drei 'Others' und drei 'Abwesenheiten' je eine", async () => {
		// Genau der gemeldete Zustand: jedes Gerät hatte eigene Zufalls-Ids vergeben.
		...
		// Die echte Aktivität bleibt unangetastet.
...
describe("Reparatur der eingebauten Zeilen erreicht den Abgleich", () => {
	it("merkt umgehaengte Eintraege und geloeschte Zeilen in der Outbox vor", async () => {
		...
		reset();
		// Ein bereits abgeglichener Eintrag: er trägt `rev`. Genau solche sammelt
		// rememberUnstamped() NICHT mehr ein - ohne Schreib-Haken bliebe das
		// Umhängen also auf diesem Gerät stehen.
		files.set(
		...
		try {
			...
			// Die Duplikate müssen als Löschung hochgehen, sonst holt sie der
			// nächste Abgleich zurück.
...
describe("Reparatur laeuft nicht zweimal ueber dieselbe Liste", () => {
	it("erzeugt bei gleichzeitigen Durchlaeufen keine neuen Duplikate", async () => {
		...
		// Abgleich, Tray). Zwei Durchläufe lesen dann dieselbe Liste, und der
		// zweite hängt seine Zeile an die des ersten an - dasselbe Duplikat, das
	...
	it("gibt der zusammengefuehrten Zeile keinen fremden Stempel mit", async () => {
		...
		// Zeile meldete das Gerät eine Änderung an einer Fassung, die der Server
