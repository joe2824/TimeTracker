...
describe("nameFromSubject", () => {
	...
	it("findet den Namen auch VOR dem Betreff, ohne Trennstrich", () => {
		...
		expect(nameFromSubject("Joel Klein Stundenerfassung Juli 2026", "Stundenerfassung")).toBe(
			"Joel Klein"
		);
	});
...
describe("buildTeamSummary", () => {
	...
	it("erkennt das Teammitglied, egal wo sein Name im Betreff steht", () => {
		// Der reale Fall: Absender gesperrt, Name vorne, Teameintrag nur "Joel".
		const eigenes: TeamMember[] = [
			{ id: "j", name: "Joel", email: "joel.klein@fresenius-kabi.com" },
			{ id: "h", name: "Hendri", email: "hendrik.solz@fresenius-kabi.com" }
		];
		const s = buildTeamSummary(
			"2026-07",
			[
				mail({
					subject: "Joel Klein Stundenerfassung Juli 2026",
					senderName: "",
					senderEmail: ""
				})
			],
		...
		expect(s.entries[0].name).toBe("Joel");
	...
	it("nimmt bei mehreren passenden Teamnamen den genaueren", () => {
		const s = buildTeamSummary(
			"2026-07",
			[mail({ subject: "Joel Klein Stundenerfassung Juli 2026", senderName: "", senderEmail: "" })],
			[
				{ id: "kurz", name: "Joel", email: "a@firma.de" },
				{ id: "lang", name: "Joel Klein", email: "b@firma.de" }
			],
			"Stundenerfassung"
		);
