...
// Steht im Einrichtungs-Assistenten und in den Kontöinstellungen.
...
export class PairingFlow {
	...
	async start(serverUrl: string): Promise<void> {
		// Ein zweiter Anlauf, während der erste noch nachsieht: ohne das liefe
		// dessen Timer weiter, und niemand hätte ihn mehr in der Hand.
		this.stop();
	...
	async #check(): Promise<void> {
		// Ohne die Sperre startet der naechste Zwei-Sekunden-Takt eine zweite
		// Anfrage, waehrend die erste noch laeuft (z.B. bei einer langsamen
		// Verbindung) - der Server loescht den Kopplungseintrag beim ersten
		// erfolgreichen Abruf, die zweite Anfrage saehe dann "abgelaufen" und
		// meldete einen Fehlschlag fuer eine tatsaechlich erfolgreiche Kopplung.
