...
describe("toTs / noonTs", () => {
	...
	it("noonTs bleibt an DST-Grenzen auf dem gemeinten Tag", () => {
		// Genau dafür ist der Mittag da: um Mitternacht kippen manche Zonen
...
describe("splitAtMidnight", () => {
	...
	it("teilt an Mitternacht – genau Annas 23:57–01:01", () => {
...
describe("splitAtMidnight – manuelle Einträge", () => {
	it("teilt einen von Hand angelegten Eintrag 23:00–01:00", () => {
		// Derselbe Fall wie beim Timer, nur über „Eintrag" oder „Mehrere Tage":
		// ohne Teilung zählte die ganze Spanne auf den Starttag.
	...
	it("startOfNextDay trifft die Grenze auch am 25-Stunden-Tag", () => {
		// +24h wäre hier eine Stunde daneben.
...
describe("quantize", () => {
	...
	it("liefert innerhalb derselben Minute denselben Wert", () => {
		// Genau darauf beruht die Ersparnis: Svelte gibt einen unveränderten
		// Wert nicht weiter, die Rechnung dahinter läuft also einmal je Minute.
		const base = wallToTs(2026, 6, 10, 14, 37, 0);
		const values = new Set<number>();
		for (let s = 0; s < 60; s++) values.add(quantize(base + s * 1000, MINUTE_MS));
		expect(values.size).toBe(1);
		// Die nächste Sekunde fällt in die nächste Minute.
