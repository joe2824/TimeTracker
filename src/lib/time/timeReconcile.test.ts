...
describe("reconcile", () => {
	...
	it("meldet einen Tag Zeitausgleich nicht als zu viel erfasste Zeit", () => {
		// An einem abgefeierten Tag stempelt niemand: LOGA meldet 0 Stunden. Zählt
	...
	it("kappt einen offenen Eintrag am Tagesende", () => {
		// Vergessener Timer in einem alten Monat: zählt nur bis Mitternacht, nicht bis heute.
		const open: Entry = {
			id: "open",
			activityId: PROJECT_DE,
...
describe("occupiedIntervals / freeIntervals", () => {
	...
	it("rechnet an der Zeitumstellung mit der Wanduhr", () => {
		// 25.10.2026 ist die Rückstellung: der Tag hat 25 Stunden. Über die
		// Differenz zum Tagesbeginn gerechnet läge 14:00 hier bei 13:00.
...
/** Einen Tag durch reconcile schicken, um an ein vollständiges ReconcileDay zu kommen. */
...
describe("planFill", () => {
	it("legt die Pause in den Nachtrag, statt quer durch die Mittagszeit zu buchen", () => {
		// 09:10–17:35 sind 8,42 h Anwesenheit, LOGA zählt 7,67 h -> 45 min Pause.
	...
	it("kommt ohne Pausenluecke aus, wenn der Platz genau reicht", () => {
		// Anwesenheit == Nettozeit: es gibt keine Pause zu berücksichtigen.
	...
	it("schlaegt keine Ganztags-Abwesenheit vor, wo schon Projektzeit liegt", () => {
		...
		// Vorschlag beim Übernehmen mit einer Fehlermeldung ab.
	...
	it("traegt an einem Ganztags-Abwesenheitstag nichts nach", () => {
		// App-Regel: dort gibt es keine Projektzeit, addEntry würde es ohnehin ablehnen.
	...
	it("bucht keine zweite Abwesenheit neben einem halben Urlaubstag", () => {
		...
		// abgewiesen – ein ganzer Tag daneben ergäbe still 11,25 h.
	...
	it("bleibt bei einem ueber Mitternacht gestempelten Tag im Tag", () => {
		...
		// Das Fenster endet an Mitternacht – ein Eintrag darüber hinaus gehörte
		// dem Folgetag und würde beim Anlegen ohnehin geteilt.
...
describe("Zusammenspiel mit dem automatischen Pausenabzug", () => {
	...
	it("traegt die ANWESENHEIT nach, nicht die Nettozeit", () => {
		// 7,67 h eins zu eins eingetragen bekämen den Abzug ein zweites Mal und
		...
		// Keine Pausenlücke: der Block spannt die Anwesenheit auf.
...
describe("targetEntryHours", () => {
	...
	it("zielt bei aktivem Abzug auf die Anwesenheit, nicht auf die Nettozeit", () => {
		// 7,67 h netto brauchen 8,42 h erfasste Zeit – sonst bekäme die Zahl den
	...
	it("rechnet den Abzug ueber den GANZEN Tag, nicht ueber den einzelnen Eintrag", () => {
		// 1,833 h stehen schon: zusammen müssen 8,42 h herauskommen.
	...
	it("wird negativ, wenn der Tag auch ohne diesen Eintrag schon zu voll ist", () => {
		// Der Aufrufer erkennt daran, dass sich das über diesen Eintrag allein
		// nicht regeln lässt – kürzer als leer geht nicht.
...
describe("splitBlocks", () => {
	...
	it("gibt den Rundungsrest an das letzte Projekt, nichts geht verloren", () => {
		...
		// Lückenlos und in der Reihenfolge der Uhr.
...
describe("rebalanceShares", () => {
	...
	it("bleibt bei vielen kleinen Anteilen auf 100 Prozent", () => {
		// Bei individueller Rundung kämen hier 101 % heraus, und der letzte
		// Regler fiele dafür auf 0.
