...
// Die Suite läuft mit fest auf Berlin gesetzter Kontozone (siehe
// testing/pinZone.ts). Wo eine andere Zone geprüft wird, wird sie hier
// ausdrücklich gesetzt und danach zurückgestellt.
...
describe("Sommerzeit-Umstellung", () => {
	...
	it("die uebersprungene Stunde faellt nach vorn, statt in den Vortag zu kippen", () => {
		// 02:30 gibt es am 29.03. nicht. Ein naiver Rückrechner landet eine Stunde
		// früher – also noch am 28., und der Eintrag sässe am falschen Tag.
	...
	it("die doppelte Stunde nimmt das erste Vorkommen", () => {
		...
		// Das zweite Vorkommen liegt eine Stunde später und ist ein anderer Moment.
	...
	it("teilt einen Lauf ueber den 25.10. in Tagesstuecke von 25 und x Stunden", () => {
		...
		// Kein Stück darf über seine eigene Mitternacht hinausragen.
...
describe("Kalenderrechnung", () => {
	...
	it("addCalendarDays ueberspringt keinen Tag an einer Umstellung", () => {
		// Über beide deutschen Umstellungen hinweg jeden Tag einmal.
		let date = "2026-03-27";
		const seen: string[] = [];
		for (let i = 0; i < 5; i++) {
			seen.push(date);
...
describe("Unabhaengigkeit von der Geraetezone", () => {
	it("derselbe Moment ergibt in verschiedenen Kontozonen verschiedene Tage", () => {
		// Der Sinn der ganzen Übung: der Kalendertag hängt an der KONTOzone.
...
describe("Sommerzeit in einer Zone mit halbstuendigem Abstand", () => {
	...
	// fällt damit MITTEN in eine UTC-Stunde: 02:00 Ortszeit sind 16:30 UTC.
	const TZ = "Australia/Adelaide";

	it("trifft die Zeit nach dem Wechsel, auch wenn davor schon gerechnet wurde", () => {
		inZone(TZ, () => {
			// Erst eine Uhrzeit VOR dem Wechsel - sie füllt den Puffer.
