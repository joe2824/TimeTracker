...
/** Die Zeitzone des Geräts. Vorbelegung beim ersten Start. */
 ...
 * behalten – eine kaputte Zone würde sonst jede Datumsrechnung der App kippen.
 *
 * @returns ob die Zone übernommen wurde
 */
...
export function zonedParts(ts: number, tz = currentZone): ZonedParts {
	...
	// die Namen hängen an der Sprache, die Zahl nicht.
...
function zoneOffsetMs(ts: number, tz = currentZone): number {
	...
	// Die Millisekunden fallen bei der Formatierung weg und müssen zurück, sonst
	// wäre der Abstand um bis zu 999 ms daneben.
 ...
 *  - Die übersprungene Stunde im Frühjahr gibt es nicht; sie fällt nach vorn
...
export function wallToTs(
	...
	// Beide möglichen Abstände holen – einen weit vor und einen weit nach dem
	...
	const a = wall - before;
	const b = wall - after;
	// Ein Kandidat zählt nur, wenn die Zone an IHM auch wirklich den Abstand hat,
	...
	if (aOk) return a;
	if (bOk) return b;
	// Lücke: die Wanduhrzeit wurde übersprungen. Nach vorn auf den ersten
...
/** Nur für Tests: die gepufferten Abstände vergessen. */
