...
/** Ein Eintrag ab 08:00 über `hours` Stunden. */
...
describe("dataFromEntries", () => {
	it("nimmt den fruehesten erfassten Tag, nicht den Monatsersten", () => {
		// Der Monatserste erfände hier 26 Werktage mit null Stunden und drückte
		// den Schnitt genau dort, wo jemand mit dem Erfassen anfängt.
...
describe("dayFacts", () => {
	...
	it("laesst Abwesenheiten aus dem Anwesenheitsfenster", () => {
		// Abwesenheiten liegen als Punkt auf der Tagesmitte. Zählten sie mit,
		// begänne der Tag um 12:00 und die Ruhezeit wäre frei erfunden.
	...
	it("zaehlt nur Unterbrechungen ab 15 Minuten als Pause", () => {
		// Zwei Lücken von je zehn Minuten sind keine zwanzig Minuten Pause,
		// sondern gar keine – der Block läuft durch (§ 4 Satz 2).
		const e = [
...
describe("avgWindow", () => {
	...
	it("mittelt gesetzlich ueber Mo–Sa und liegt damit deutlich tiefer", () => {
		...
		// Das ist der Grund für die zweite Lesart: hier ist die Grenze weit weg.
	...
	it("meldet eine zu kurze Datenbasis, statt sie mit Null-Tagen aufzufuellen", () => {
		...
		// Der Schnitt bleibt bei 7,5 – erfundene Null-Tage hätten ihn gedrückt.
		expect(w.average).toBeCloseTo(7.5);
	});
...
describe("forecast", () => {
	...
	it("findet den Tag, an dem der Schnitt bei gleichbleibendem Tempo reisst", () => {
		...
		// hält die leichtere Vergangenheit den Schnitt unten.
		expect(f.crossing!.date > stepDate(UNTIL, 30)).toBe(true);
	});

	it("liefert das hoechste vertraegliche Tempo exakt", () => {
		// Ist das Fenster irgendwann ganz mit künftigen Tagen gefüllt, steht
	...
	it("stuft nach Umkehrbarkeit, nicht nach Ueberschreitung", () => {
		// Über der Grenze zu liegen ist für sich kein Notfall – das Fenster
		// rollt, und wer kürzer tritt, holt es wieder ein. Gestuft wird deshalb
		// danach, ob es noch zu drehen ist.
		expect(forecast(facts(7.5), "strict", { ...base, pace: 7.5 }).verdict.level).toBe("ok");

		// Läuft hinein, aber der Umkehrpunkt ist weit: Beobachtung, kein Auftrag.
	...
	it("zeichnet die Vergangenheit nur so weit, wie ein volles Fenster reicht", () => {
		// Mit 200 Tagen Daten traegt genau der Rest über 168 Tage hinaus einen
		// vollständigen Rückblick – mehr darf die Kurve nicht behaupten.
	...
	it("faengt erst am Stichtag an, wenn kein volles Fenster in der Vergangenheit liegt", () => {
		// Sonst stünde ein halb ausserhalb der Datenbasis liegendes Fenster als
		// tiefer Punkt in der Kurve und sähe aus wie eine Entspannung, die es
		// nie gab.
		const from = stepDate(UNTIL, -55);
		const f = forecast(
			dayFacts(series(from, UNTIL, 7.5), ABSENCE, { deductBreaks: false }),
...
describe("dayFindings", () => {
	...
	it("prueft die Ruhepause nicht, solange der Abzug aktiv ist", () => {
		// Sieben Stunden Timer am Stück, ohne erfasste Pause. Mit aktivem Abzug
		// rechnet die App wie LOGA – das darf keinen Verstoss auslösen, sonst
		// wäre es der häufigste Befund und der falscheste.
	...
	it("misst die Ruhezeit auch ueber die Monatsgrenze", () => {
		...
		expect(f.find((x) => x.rule === "restPeriod")?.level).toBe("violation");
		// Der Befund gehört an den Tag, an dem zu frueh angefangen wurde.
...
describe("checkArbZg", () => {
	...
	it("meldet einen gerissenen Schnitt im Urteil, nicht als Tagesbefund", () => {
		...
		// doppelten Schlüssel in einer nach Regel+Tag adressierten Liste.
...
// ---------------------------------------------------------------------------
// Die Zeitsimulation gegen eine unabhängige Referenzrechnung.
...
/** Der Schnitt eines Fensters, naiv Tag für Tag. */
...
/** Ein zufälliges halbes Jahr: unterschiedlich lange Tage, Urlaub, freie Tage. */
function scenario(seed: number, until = UNTIL): { entries: Entry[]; dataFrom: string } {
	...
	for (let d = from; d <= until; d = stepDate(d, 1)) {
		...
		if (!MO_FR.includes(wd)) {
			// Gelegentlich Samstagsarbeit – die zählt im Zähler, gibt aber in der
...
describe("Zeitsimulation", () => {
	...
	it("haelt die Zusage von maxPace – und was sie nicht verspricht", () => {
		// Die Empfehlung lautet "höchstens X h je Arbeitstag". Zwei Dinge muss sie
		// leisten:
		//
		// 1. Ab vier Wochen Vorlauf hält sie jedes Fenster unter acht Stunden.
		// 2. Davor darf sie streifen – die Fenster dort sind von bereits
		//    gearbeiteten Stunden bestimmt, und eine Tempömpfehlung kann daran
		//    wenig ändern. Aber sie darf nicht beliebig danebenliegen: eine
		...
		for (let seed = 40; seed <= 47; seed++) {
			...
			// Und die Empfehlung ist nicht unnötig streng: eine Viertelstunde mehr
			// je Tag reisst es.
			const over = forecast(facts, "strict", { ...opts, pace: f.maxPace + 0.25 });
			expect(over.peak.average, `seed ${seed}`).toBeGreaterThan(NORM_DAILY);
		}
		// Der zweite Teil muss auch wirklich geprüft worden sein.
		expect(checkedNearTerm).toBeGreaterThan(0);
	});

	it("laesst maxPace nicht vom naechsten Tag bestimmen", () => {
		// Das Fenster, das MORGEN endet, hat genau einen künftigen Arbeitstag im
		// Nenner - das gesamte Restbudget fällt auf ihn und würde maxPace bei
	...
	it("faellt beim Tempo auf das volle Fenster zurueck, wenn zuletzt nur Urlaub war", () => {
		...
		// Rückfall wäre das Tempo 0:00 und die Auskunft „im grünen Bereich,
		// Luft 8:00 h je Arbeitstag" – ausgerechnet für jemanden, der bei 7:54
		...
		// Ohne Rückfall wäre hier 0 herausgekommen.
	...
	it("setzt das Entlastungsdatum hinter die LETZTE Ueberschreitung", () => {
		...
		// während der Schnitt in vier Wochen noch einmal darüber geht, weil ein
		// leerer Tag hinten aus dem Fenster fällt. Geprüft wird gegen die naive
		// Rechnung ohne jede künftige Stunde.
	...
	it("bestimmt den Umkehrpunkt exakt – am naechsten Arbeitstag traegt es nicht mehr", () => {
		...
		// um eins danebenliegen kann, ohne dass es auffällt. Geprüft wird er
		// deshalb gegen dieselbe naive Rechnung: bis zum Umkehrpunkt im aktuellen
		// Tempo, danach nichts – dann muss der Höchststand halten, und am
		// nächsten ARBEITSTAG darf er es nicht mehr. Nicht am nächsten
		// Kalendertag: ein Wochenende weiterzuarbeiten ändert nichts, dort hält
		// es also zwangsläufig weiter.
		const limit = NORM_DAILY + AVG_TOLERANCE;
		let checked = 0;
		...
		for (let seed = 60; seed <= 75; seed++) {
			...
			const facts = dayFacts(entries, ABSENCE, { deductBreaks: false });
			const opts = { until: UNTIL, dataFrom, workdays: MO_FR };
			// Ein Tempo deutlich über der Grenze erzwingt einen Umkehrpunkt.
			const pace = 9.5;
...
describe("Kalender-Randfaelle", () => {
	...
	it("zaehlt in jedem 168-Tage-Fenster exakt 120 bzw. 144 Werktage", () => {
		for (const [until, why] of CUTOFF_DATES) {
			...
			// über Millisekunden, und an der Zeitumstellung hat ein Tag 23 oder 25
			// Stunden. 168 Tage müssen trotzdem 24 Wochen ergeben.
	...
	it("verbucht Stunden am Schalttag", () => {
		...
		// Und der Tag danach ist der 1. März, nicht der 30. Februar.
	...
	it("stimmt auch ueber Schalttag und Zeitumstellung mit der naiven Rechnung ueberein", () => {
		...
		// den 29.02.2024 und die Umstellung vom 31.03.2024 enthält.
...
describe("boesartige Eintraege", () => {
	...
	it("laesst eine Abwesenheit am Wochenende das Budget nicht mindern", () => {
		...
		// Nicht-Arbeitstagen ausdrücklich (report.ts); die gesetzliche Lesart
		// zählt den Samstag aber als Werktag – ohne dieselbe Regel verschwände
	...
	it("summiert ueberlappende Eintraege, sieht sie aber als EINEN Block", () => {
		...
		// kommt über Kalender-Import und Nachtrag trotzdem vor. Die Stunden
		// addieren sich – aber für § 4 ist es durchgearbeitete Zeit, keine Pause.
	...
	it("schlaegt einen Eintrag ueber Mitternacht seinem STARTTAG zu", () => {
		...
		// zählte dieselbe Stunde zweimal im Fenster.
		const e: Entry = {
			id: "x",
			activityId: "a",
	...
	it("kappt einen vergessenen laufenden Eintrag am Ende SEINES Tages", () => {
		// Sonst wächst ein liegen gebliebener Timer bis heute weiter und sprengt
	...
	it("hebt den Schnitt durch Sonntagsarbeit, ohne Budget dafuer zu geben", () => {
		// Der Sonntag ist kein Werktag, seine Stunden zählen aber sehr wohl. Wer
	...
	it("misst an der Zeitumstellung die echte Dauer, nicht die Wanduhr", () => {
		...
		// aber genau hier fällt es auf, wenn jemand auf Wanduhrzeiten umstellt.
