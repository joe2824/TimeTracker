...
export interface ReconcileOptions {
	...
	hoursPerDay: number;
	/** Ab welcher Abweichung ein Tag auffällt, in Stunden */
	tolerance: number;
...
export function reconcile(
	...
	for (const rawReport of days) {
		...
		if (reportHours <= 0) {
			// Wochenende/frei. Nur auffällig, wenn hier trotzdem Zeit steht.
...
export function occupiedIntervals(entries: Entry[], date: string, now: number): Interval[] {
	...
	for (const e of entries) {
		...
		// Über Mitternacht: an der Tagesgrenze kappen. Genau dort wäre die
...
/**
 * Aus den freien Spannen abwechselnd nehmen und überspringen.
		...
		while (left > 0 && idx < free.length) {
			...
			if (use > 0) {
				if (step.take) {
					...
					// Zwei Stücke, die exakt aneinander stossen, sind eines.
...
export function planFill(
	...
	if (day.looksLikeAbsence) {
		...
		// zählt: ein zweiter Eintrag daneben würde nicht abgewiesen, sondern
		...
		// stünde hier ein Vorschlag, den addEntry beim Übernehmen abweist – bei
		// einer Sammelübernahme für jeden solchen Tag mit eigener Fehlermeldung.
	...
	// Zieht die App die Pause selbst ab, ist die Zielgrösse die ANWESENHEIT:
	// LOGA-Stunden sind netto, eins zu eins eingetragen bekämen sie den Abzug
	...
	if (hasStamps(day.report)) {
		...
		// liesse sich so ein Tag nie vollständig nachtragen.
	...
	let blocks: Interval[];
	// Keine Lücke aussparen, wenn der Abzug sie ohnehin erledigt oder das
	// Fenster ohnehin schon über die Stempelzeiten hinaus verlängert wurde –
	// dort ist von der ursprünglichen Tagesform nichts mehr übrig.
	...
	// aber nicht gestempelt – und gehört damit meist auf eine andere Aktivität.
