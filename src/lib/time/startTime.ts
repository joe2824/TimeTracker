...
export function resolveStartTs(
	...
	if (customStart) {
		...
		// In der Zeitzone des Kontos, nicht der des Geräts: der Startzeitpunkt
		// wird zu einem Eintrag, und dessen Tag muss auf jedem Gerät derselbe sein.
		const today = fmtDate(now);
		let ts = toTs(today, minToClock(min));
		// Uhrzeit heute noch nicht erreicht -> gemeint ist gestern. Über den
