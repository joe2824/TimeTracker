...
//   Timer, der über die Mittagspause durchläuft, dürfte dann keinen Verstoss
//   auslösen. Das wäre der häufigste Befund gewesen, und zwar ein falscher.
// - Feiertage (§ 9), weil die App keinen Feiertagskalender hat. Ein gebuchter
//   Feiertag ist eine Ganztags-Abwesenheit und fällt damit ohnehin aus der
//   Rechnung.
// - Nachtarbeit (§ 6) und Ersatzruhetage (§ 11) – für Büroarbeit totes Gewicht.
//
...
/**
 * Wie weit der Schnitt über acht Stunden liegen muss, damit es als
 * Überschreitung gilt – drei Minuten je Werktag.
 */
...
/**
 * So viele künftige Arbeitstage muss ein Fenster enthalten, damit es das
 * höchste verträgliche Tempo mitbestimmt.
 */
...
/** Kürzeste Unterbrechung, die nach § 4 Satz 2 als Pause zählt (Minuten). */
 ...
 *   Werktag, auch wenn niemand an ihm arbeitet. Bei einer Fünf-Tage-Woche mit
 *   7,5 h ergibt das rund 250 Stunden Puffer je Fenster; die Grenze ist damit
 *   praktisch unerreichbar, und die Linie läge flach weit unter acht Stunden.
 * - "strict": nur die eingestellten Arbeitstage. Keine Gesetzeslage, sondern
 *   der Frühwarnwert – nur diese Rechnung warnt früh genug, um noch etwas
 *   ändern zu können. Die Oberfläche muss beide zeigen und benennen, welche
 *   welche ist.
 */
...
export interface ArbZgFinding {
	...
	/** "YYYY-MM-DD" */
	date: string;
	/** Kurzform für ein Badge, z.B. "> 10 h" */
	label: string;
	/** Ein Satz Klartext. */
	text: string;
	/** Gemessener Wert in Stunden, wo einer existiert. */
	value?: number;
	/** Grenzwert in Stunden, gegen den geprüft wurde. */
	limit?: number;
}

/** Was ein einzelner Kalendertag zur Prüfung beiträgt. */
export interface DayFacts {
	 ...
	 * Pausenabzug aktiv ist – dann wird § 4 nicht geprüft und die Zahl wäre
	 * eine Aussage über Lückenhaftigkeit der Erfassung, nicht über Pausen.
	 */
	pauseMinutes: number | null;
	/** Längster Block ohne solche Unterbrechung, in Stunden. Null wie oben. */
...
export interface AvgWindow {
	...
	/** Stichtag "YYYY-MM-DD". */
	to: string;
	/** Werktage im Fenster, abzüglich gebuchter Abwesenheiten. */
	budgetDays: number;
	/** budgetDays * 8 */
	budgetHours: number;
	actualHours: number;
	...
	bufferHours: number;
	/** Reicht die Datenbasis über das ganze Fenster? */
	complete: boolean;
	/** Tatsächlich abgedeckte Wochen – nur interessant, wenn !complete. */
	weeksCovered: number;
}

/** Ein Punkt der Verlaufskurve für das Diagramm. */
...
export interface Forecast {
	...
	pace: number;
	horizonWeeks: number;
	/** Wochenweise abgetastet – für das Diagramm, nicht für die Entscheidungen. */
	points: ForecastPoint[];
	/** Erster Tag, an dem der Schnitt bei gleichbleibendem Tempo über 8 h liegt. */
	crossing: { date: string; average: number } | null;
	/** Höchststand im Prognosezeitraum. */
	peak: { date: string; average: number };
	/**
	 * Grösstes konstantes Tempo (h je Arbeitstag), mit dem der Schnitt im
	 * ganzen Zeitraum unter acht Stunden bleibt. Null, wenn kein Fenster in der
	 * Zukunft einen Arbeitstag enthält (dann lässt sich nichts mehr steuern).
	 */
	maxPace: number | null;
	 ...
	 * halten wäre: der Tag, an dem das Fenster auch ohne jede weitere Stunde
	 * wieder unter acht Stunden fällt.
	 */
	reliefDate: string | null;
	/**
	 * Der späteste Tag, an dem man anfangen kann herunterzugehen, ohne die
	 * Grenze zu reissen – der Umkehrpunkt.
	 */
	easeOffDate: string | null;
	/** Selbst ab sofort nichts mehr zu arbeiten wendet die Überschreitung nicht ab. */
	tooLate: boolean;
	/** Kurzurteil für den ersten Blick. */
	verdict: Verdict;
}
 ...
 * Wörter, dann die eine Zahl, auf die es ankommt.
 */
export interface Verdict {
	level: VerdictLevel;
	/** Kurzurteil, vier bis sechs Wörter. */
	headline: string;
	/** Ein Satz mit der Zahl, die zählt. */
	detail: string;
	/**
	 * Ist etwas zu TUN – also müsste man sp&uuml;rbar herunter, um das Fenster zu
	 * halten?
	 */
...
export interface ArbZgOptions {
	...
	until: string;
	/** Ab diesem Datum gelten die Einträge als vollständig geladen. */
	dataFrom: string;
	/** Reguläre Arbeitstage als Wochentagsnummern (settings.workdays). */
	workdays: number[];
	/** settings.breakDeduction – schaltet die § 4-Prüfungen. */
...
/** Ab wann die Daten als belastbar gelten: der frühest ERFASSTE Tag. */
...
export function dayFacts(
	...
	for (const e of entries) {
		...
		// Anwesenheitsfenster verfälschen – deshalb erst hier, nach dem continue.
	...
	// Pausen nur, wo sie auch geprüft werden – siehe Kommentar an DayFacts.
	if (!deductBreaks) {
...
/**
 * Eine lüchenlose Datumsachse von `from` bis `to` mit allem, was die
...
function buildAxis(
	...
	for (const date of dates) {
		...
		// Vor dem Beginn der Datenbasis trägt ein Tag NICHTS bei – weder Stunden
		// noch Budget. Sonst verdünnten lauter erfundene Null-Tage den Schnitt und
		// die Auskunft wäre beruhigend statt richtig.
...
/** Die Zeitachse samt allen Präfixsummen – einmal aufgebaut. */
export interface Prepared {
	dates: string[];
	todayIndex: number;
	/** Präfixsummen der Stunden. */
	hours: number[];
	/** Präfixsummen der Budgetanteile, je Lesart. */
	budget: Record<AvgBasis, number[]>;
...
export function prepare(
	...
	// Doppelt so weit zurück wie das Fenster lang ist: ein Fenster, das VOR dem
	// Stichtag endet, braucht seinerseits 168 Tage Vorlauf. Nur so lässt sich
...
/** Summe über [lo, hi] beider Grenzen einschliesslich, geklemmt auf die Achse. */
...
export function forecast(
	...
	// Gezeichnet wird nur rückläufig, soweit die Daten ein VOLLES Fenster
	// hergeben. Ein Fenster, das zur Hälfte vor dem ersten erfassten Tag liegt,
	// stünde sonst als tiefer Punkt in der Kurve und sähe aus wie eine
	...
	for (let i = startIndex; i < axis.dates.length; i++) {
		...
		// Höchstes verträgliches Tempo aus genau diesem Fenster.
		const lo = i - (AVG_WINDOW_DAYS - 1);
		...
		// Entlastung: ab wann trägt das Fenster wieder, selbst ohne jede weitere
		// Stunde? Nur diese Frage bleibt, wenn maxPace ins Negative fällt.
	}
	...
	/** Höchststand im Prognosezeitraum, wenn ab `stopIndex + 1` nichts mehr kommt. */
	...
	// Je später der Aufhörtag, desto mehr lange Tage im Fenster – der
	// Höchststand wächst also monoton mit `stopIndex`. Damit findet die
	// Halbierung den letzten Tag, der noch trägt, in acht Schritten.
	...
	if (!tooLate && !holds(last)) {
		let lo = axis.todayIndex - 1; // hält
		let hi = last; // hält nicht
		while (hi - lo > 1) {
			const mid = Math.floor((lo + hi) / 2);
			if (holds(mid)) lo = mid;
			else hi = mid;
		}
		// Auf den letzten ARBEITSTAG zurückgehen.
	...
	/** Ab wann trägt das Fenster wieder, selbst ohne jede weitere Stunde? */
...
function makeVerdict(f: {
	...
	// Schon drüber – das ist keine Prognose mehr, sondern der Stand.
	...
	if (f.easeOffDate) {
		...
		// Der Umkehrpunkt rückt in Reichweite: jetzt ist etwas zu tun.
	...
	// Kein Umkehrpunkt nötig – das Tempo trägt das Fenster dauerhaft.
...
export function dayFindings(
	...
	// Für § 5 zählt das Ende des letzten Tages, an dem überhaupt erfasst
	// wurde – nicht das des Vortags. Nach einem freien Tag ist die Ruhezeit
	// längst erfüllt, die Rechnung ergibt dann von selbst einen grossen Wert.
...
/**
 * Der komplette Check für die Oberfläche.
 *
...
export function checkArbZg(entries: Entry[], opts: ArbZgOptions): ArbZgResult {
	...
	// Die Achse einmal – sie ist für alle fünf folgenden Rechnungen dieselbe.
	...
	// Der gerissene Durchschnitt ist bewusst KEIN Tagesbefund: er gehört keinem
	// Tag, sondern fällt am Stichtag an und galt genauso am Tag davor. Als Zeile
	// in "Auffällige Tage" stünde er je Lesart einmal am selben Datum - und weil
	// die Liste nach Regel adressiert wird, wäre das ein doppelter Schlüssel.
	const windows = {
