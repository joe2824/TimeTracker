// Automatischer Pausenabzug. Berührt die gespeicherten Einträge NICHT - abgezogen
// wird erst beim Aufsummieren.

/**
 * Die Stufen der Regel, ABSTEIGEND nach Schwelle – breakDeduction nimmt die
 * erste passende. Eine dritte Stufe liesse sich hier einhängen, ohne dass
 * grossForNet oder die Verteilung etwas davon mitbekommen müssten.
 */
...
/** Der Pausenabzug für eine Tages-Arbeitszeit, in Stunden. */
...
/**
 * Den Pausenabzug eines Tages anteilig auf dessen Aktivitäten verteilen - anteilig,
 * damit das Ergebnis nicht von der Reihenfolge der Einträge abhängt.
 *
 * @param perActivity Stunden je Aktivität EINES Tages (ohne Abwesenheiten)
 * @returns dieselbe Aufschlüsselung, abzüglich der Pause
 */
export function deductBreakFromDay(perActivity: Map<string, number>): Map<string, number> {
	...
	// Faktor kann also nie negativ werden. Die Wache steht für den Fall, dass
	// jemand die Schwellen später verstellt.
...
/** Tagesarbeitszeit abzüglich Pause. Für Summen ohne Aufschlüsselung. */
...
export function grossForNet(netHours: number): number {
	...
	// erste Stufe, die sich selbst bestätigt.
