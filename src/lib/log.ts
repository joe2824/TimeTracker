// Datei-Protokoll für die Fehlersuche nach dem Fakt.
...
/** So viele Tage bleiben liegen; ältere räumt pruneOldLogs() beim Start weg. */
...
/** Zusatzangaben einer Zeile. Bei Fehlern möglichst mit Aufrufliste. */
...
async function writePending(): Promise<void> {
	...
	pending = [];
	try {
		// Einmal je Sitzung prüfen: das Protokoll schreibt oft, und jede Prüfung
		// wäre eine weitere Runde über die IPC-Brücke.
		if (!dirReady) {
		...
		// Deshalb auch kein erneuter Versuch: dann hänge das Protokoll im Kreis.
		console.error("Protokoll konnte nicht geschrieben werden", e, lines);
	}
}

/** Alles Angesammelte wegschreiben. Schreibvorgänge laufen nacheinander. */
...
/** Die letzten Zeilen des Protokolls, älteste zuerst. */
...
/** Alle Protokolldateien löschen. Liefert die Anzahl. */
...
/** Protokolle älter als `keepDays` entfernen. Liefert die gelöschten Namen. */
