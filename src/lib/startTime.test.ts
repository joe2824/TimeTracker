...
const NOW = wallToTs(2026, 7, 8, 14, 0, 0);
...
describe("resolveStartTs", () => {
	...
	it("eine gesetzte Uhrzeit überschreibt das Preset", () => {
		...
		expect(resolveStartTs(30, "13:00", NOW)).toBe(wallToTs(2026, 7, 8, 13, 0, 0));
	});

	it("noch nicht erreichte Uhrzeit meint gestern", () => {
		// 15:00 ist um 14:00 noch nicht gewesen -> gemeint ist gestern 15:00.
		expect(resolveStartTs(0, "15:00", NOW)).toBe(wallToTs(2026, 7, 7, 15, 0, 0));
	});
	...
	it("greift ueber DST-Grenzen auf den Vortag (25-Stunden-Tag)", () => {
		...
		const nach = wallToTs(2026, 10, 26, 1, 0, 0);
		const p = zonedParts(resolveStartTs(0, "23:00", nach)!);
