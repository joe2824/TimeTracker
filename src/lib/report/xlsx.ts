// Minimaler XLSX-Leser für das erste Arbeitsblatt einer Datei. Ohne Formeln,
...
export interface XlsxSheet {
	...
	name: string;
	/**
	 * Zeilen als Text. Lücken (fehlende Zellen, übersprungene Zeilen) sind "",
	 * damit der Spaltenindex über alle Zeilen derselbe bleibt.
	 */
	rows: string[][];
}

/** Die Datei ist kein lesbares XLSX. Trägt eine Meldung für die Oberfläche. */
...
/** Maximale Grösse des ZIP-Kommentars (u16) + EOCD-Kopf selbst. */
const EOCD_MAX_SEARCH = 0xffff + 22;

/** Alle Einträge eines ZIP-Archivs als Name -> Rohdaten (noch gepackt). */
function readZipEntries(buf: Uint8Array): Map<string, { method: number; data: Uint8Array }> {
	const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);

	// EOCD von hinten suchen – davor darf ein Kommentar beliebiger Länge stehen.
	let eocd = -1;
	...
	// ab 4 GB oder 65535 Einträgen – für einen Zeitwirtschaftsreport unmöglich.
	...
	for (let i = 0; i < count; i++) {
		...
		// Der lokale Kopf hat sein EIGENES Extra-Feld, das länger sein darf als das
...
/** XML-Entities auflösen, inklusive numerischer (&#10; / &#x1F;). */
...
/** Text aller <t>-Elemente eines Abschnitts, aneinandergehängt (Rich Text). */
...
/** "A" -> 0, "Z" -> 25, "AA" -> 26. -1 bei ungültiger Referenz. */
...
export function parseSheetXml(xml: string, shared: string[]): string[][] {
	...
	// Für Zeilen ohne `r`: fortlaufend weiterzählen.
	let nextRow = 1;
