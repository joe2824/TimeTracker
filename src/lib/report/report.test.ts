...
// Lokalzeit, nicht Date.UTC: buildReport gruppiert über fmtDate und prüft
// Werktage über getDay() – beides lokal. Mit UTC-Zeitstempeln entschiede die
// Zone des Rechners, auf welchen Tag (und Wochentag) ein Eintrag fällt.
...
/** Tagesversatz über den Kalender, nicht über +24h – siehe allDayNoons. */
...
describe("buildReport", () => {
	...
	it("ignoriert Projektzeiten an Ganztags-Abwesenheitstagen", () => {
		...
		expect(hours["Projekt 1"]).toBe(0); // Projektzeit fällt weg
...
describe("buildReport – Zeitausgleich", () => {
	...
	it("zaehlt einen Zeitausgleich am Wochenende nicht mit", () => {
		// Am freien Tag gibt es kein Tagessoll, das offen bleiben könnte.
...
describe("buildReport – offene Einträge", () => {
	...
	it("kappt einen vergessenen offenen Eintrag am Ende SEINES Tages", () => {
		// Ohne Kappung zählte ein offener Eintrag bis Date.now(): ein vergessener
...
describe("buildReport mit automatischem Pausenabzug", () => {
	/** Eintrag an einem frei wählbaren Tag im Juni 2026. */
	...
	it("schwellt je Tag, nicht auf der Monatssumme", () => {
		...
		// Auf der Monatssumme geschwellt entstünde hier fälschlich ein Abzug.
	...
	it("zieht an einem halben Urlaubstag nur von der Projektzeit ab", () => {
		...
		// die 3,75 h Abwesenheit bleiben unberührt.
