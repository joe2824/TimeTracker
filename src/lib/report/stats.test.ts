...
/** Eintrag am lokalen Tag `d` (Monat 1-basiert) über `hours` Stunden ab 08:00. */
...
describe("dayWorkHours", () => {
	...
	it("kappt einen vergessenen offenen Eintrag am Ende SEINES Tages, statt bis now zu zaehlen", () => {
		...
		// Jahres-Heatmap sprengen: normalisiert wird auf den stärksten Tag, ein
...
describe("dayActivityHours", () => {
	it("schluesselt einen Tag nach Aktivitaet auf", () => {
		...
		expect(day.get("a")).toBeCloseTo(5); // zwei Einträge derselben Aktivität summiert
...
describe("heatmapYear", () => {
	...
	it("laesst aehnliche Arbeitstage aehnlich aussehen", () => {
		// Realistische Streuung: Quartile würden hier über alle 4 Stufen spreizen.
		const byDay = new Map([
			["2026-06-01", 7.2],
			["2026-06-02", 7.5],
			["2026-06-03", 7.9],
...
describe("heatmapYear – Monatsanfaenge (Beschriftung der Heatmap)", () => {
	// Die Card sucht je Wochenspalte einen Tag, der auf den Monatsersten fällt.
	// Nur den ersten Tag der Spalte zu prüfen würde Monate übersehen, die
	// mitten in der Woche starten (es blieben sonst nur Jan und Jun übrig).
