...
export interface MonthReport {
	...
	workHours: number;
	/**
	 * Wie viel des Monats auf Zeitausgleich entfällt.
	 *
...
/**
 * Aggregiert die Einträge eines Monats zu einer Zeile je Aktivität.
 * Es erscheinen ALLE nicht-archivierten Aktivitäten (auch mit 0 Stunden),
 * jeweils auf `step` Stunden gerundet.
 *
 * `deductBreaks` zieht die Pause JE TAG ab, nie auf der Monatssumme - die Regel
 * hängt an der Tagesarbeitszeit (siehe breaks.ts).
 */
export function buildReport(
	...
	// Abwesenheiten an Nicht-Arbeitstagen (z. B. Wochenende) zählen nicht mit –
	...
	// Der Zeitausgleich zusätzlich getrennt - nur zum Anzeigen.
	let timeOffHours = 0;
	...
	// Tage mit Ganztags-Abwesenheit: an diesen Tagen zählen Projektzeiten nicht.
...
/**
 * Reine Textfassung des Berichts – als Body für den mailto-Fallback,
 * wenn kein klassisches Outlook für die HTML-Tabelle verfügbar ist.
 */
