...
/** Liest Kalendereinträge im Zeitraum (ISO-Datum, inkl. Grenzen). */
...
/** Ermittelt (ohne COM-Start), welche Outlook-Variante verfügbar/aktiv ist. */
export async function detectOutlook(): Promise<OutlookInfo> {
	return invoke<OutlookInfo>("detect_outlook");
}

/**
 * Baut aus einem fehlgeschlagenen COM-Aufruf eine verständliche Meldung.
 * Erklärt speziell den Fall "nur neues Outlook, kein klassisches Profil".
 */
...
/**
 * Einen fehlgeschlagenen Outlook-Aufruf protokollieren und die Meldung für den
