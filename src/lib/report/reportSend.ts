 ...
 * Nur mit dem text/html-Flavor fügt ein Mailprogramm die gerenderte Tabelle
...
export async function copyReportToClipboard(html: string, text: string): Promise<ClipboardMode> {
	try {
		...
		// Ältere Webviews ohne ClipboardItem/write: Quelltext ist besser als nichts.
		try {
...
/**
 * Steht als einzige Zeile im Entwurf, solange die Tabelle noch nicht eingefügt
 ...
 * dem Hinweis öffnen – eingefügt wird von Hand. Nur wenn die Zwischenablage
 * versagt, kommt der Text ersatzweise in die Mail.
 *
 * Wirft, wenn sich das Mailprogramm nicht öffnen lässt.
 */
