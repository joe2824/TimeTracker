...
const { createBackupData, inspectBackup, restoreBackup, snapshotBeforePairing, clearSnapshots, SNAPSHOT_DIR } =
	await import("./backup");
const { clearAccountData } = await import("../store");
const { storage } = await import("../platform/fs");
...
describe("Backup & Restore", () => {
	...
	it("sichert, wenn der Abgleich nur wiederholt, was schon dasteht", async () => {
		// Ein Gerät, das der Nachlauf zurückgesetzt hat, holt die Monate ein
		// zweites Mal - sie liegen aber längst hier. Zu sperren wäre Fehlalarm.
		await saveEntries("2026-07", sampleEntries202607);
		account.backfilling = true;
		account.historyIncomplete = false;
	...
	it("stellt Daten im Ersetzen-Modus (replace) vollständig wieder her", async () => {
		...
		// Die Sicherung bringt zwei Aktivitäten mit; "Others" und "Abwesenheiten"
		// stellt die App direkt beim Einlesen wieder her - ohne sie fehlt dem
		// Bericht seine Sammelzeile.
	...
	it("stellt Daten im Zusammenführen-Modus (merge) additiv wieder her", async () => {
		...
		// Drei aus der Zusammenführung, dazu die beiden eingebauten Zeilen.
...
describe("Sicherung vor der Kopplung", () => {
	const acts: Activity[] = [
		{ id: "a1", name: "Entwicklung", color: "blue", archived: false, isAbsence: false, sortOrder: 0 }
	];
	...
	it("überlebt den Kontowechsel, der `data` wegräumt", async () => {
		await saveActivities(acts);
		const name = await snapshotBeforePairing();
		expect(name).toBeTruthy();

		// Genau darauf kommt es an: `clearAccountData` raeumt `data`, nicht `backups`.
		await clearAccountData();

		const left = (await storage.readDir(SNAPSHOT_DIR)).map((e) => e.name);
		expect(left).toContain(name);
	});

	it("legt nichts an, wenn es nichts zu verlieren gibt", async () => {
		expect(await snapshotBeforePairing()).toBeNull();
		expect(await storage.readDir(SNAPSHOT_DIR).catch(() => [])).toEqual([]);
	});

	it("behält höchstens drei", async () => {
		await saveActivities(acts);
		for (let i = 0; i < 5; i++) {
			vi.setSystemTime(new Date(2026, 8, 7, 12, 0, i));
			await snapshotBeforePairing();
		}
		expect((await storage.readDir(SNAPSHOT_DIR)).length).toBe(3);
	});

	it("die Datei lässt sich wieder einlesen", async () => {
		await saveActivities(acts);
		const name = await snapshotBeforePairing();

		const json = await storage.readTextFile(`${SNAPSHOT_DIR}/${name}`);
		expect(inspectBackup(json).valid).toBe(true);
	});
});
