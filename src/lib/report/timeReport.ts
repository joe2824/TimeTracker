...
export interface TimeReportPerson {
	/** Stabiler Schlüssel für die Auswahl: Personalnummer, sonst der Name. */
	key: string;
	personnelNo: string;
...
/** Vergleichsform einer Überschrift: ohne Rand, klein, Leerraum vereinheitlicht. */
function normalizeHeader(s: string): string {
	return s.trim().toLowerCase().replace(/\s+/g, " ");
}

/**
 * Spalten des Reports. Der Schlüssel ist intern, die Liste enthält die
 * Überschriften, unter denen die Spalte auftauchen darf.
 */
const COLUMNS = {
...
function mapHeader(row: string[]): ColumnMap | null {
	...
	for (let i = 0; i < row.length; i++) {
		...
		for (const key of Object.keys(COLUMNS) as ColumnKey[]) {
			// Erste Fundstelle gewinnt: eine doppelte Überschrift soll die schon
			// zugeordnete Spalte nicht überschreiben.
