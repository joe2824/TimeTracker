...
/** Ein Blatt aus Kopfzeile + Datenzeilen bauen, ohne den Umweg über ZIP. */
...
describe("parseReportClock", () => {
	...
	it("liest die numerische 0 als nicht-gestempelt, nicht als Mitternacht", () => {
		// Sonst bekäme ein Urlaubstag ein Anwesenheitsfenster ab 00:00, und der
		// Nachtrag schlüge Projektzeit mitten in der Nacht vor.
...
describe("Pause", () => {
	...
	it("liest die tatsaechliche Pause aus Brutto minus Netto", () => {
		...
		// Zusätzlich gestempelte Pause – mehr als jede Regel vorhersagt.
	...
	it("faellt ohne Stempel auf die Regel zurueck", () => {
		...
		// Nur ein Stempel reicht nicht für eine Messung.
		expect(breakHours(day("08:00", null, 7.5))).toBe(0);
	});

	it("nimmt die Regel, wenn die Messung nichts hergibt", () => {
		// Netto >= Brutto (Korrekturbuchung): gemessen wäre die Pause 0 oder negativ.
...
describe("echter Report (anonymisiertes Fixture)", () => {
	...
	it("unterscheidet Arbeitstage, freie Tage und Abwesenheiten", async () => {
		...
		// Kein Werktag fällt durchs Raster: freie Tage sind ausschliesslich Wochenenden.
		for (const d of free) {
