...
export function nameFromSubject(subject: string, subjectFilter = ""): string | null {
	...
	// und die Monatsangabe. Was übrig bleibt, ist der Name. Deckt die Form ab,
...
function memberFromSubject(
	...
	for (const m of team) {
		...
		// Längster Treffer gewinnt: steht "Anna" und "Anna Meier" im Team, ist
...
/** Wörter ab zwei Zeichen, kleingeschrieben. Bindestrich trennt (Meier-Schmidt). */
 ...
 * bis zum 20. des Folgemonats. Früh genug für den, der schon am 20. meldet,
 * spät genug für den Nachzügler.
 */
...
export interface TeamEntry {
	/**
	 * Eindeutig innerhalb einer Uebersicht – auch für Mails ohne erkennbaren
	 * Absender. Die Oberfläche braucht das als Listenschlüssel.
	 */
	key: string;
 ...
 * `memberId = null` geführt - beim ersten Einrichten ist die Liste leer.
 */
export function buildTeamSummary(
	...
	for (const mail of mails) {
		...
		// Wege zur Person: Adresse, dann Absendername. Der Betreff zählt NUR, wenn
		// Outlook überhaupt keinen Absender herausgegeben hat (siehe
		// nameFromSubject) – sonst gewänne bei einer weitergeleiteten Mail der Name
		// im Betreff über den tatsächlichen Absender.
		...
		// Schlüssel bewusst über die Person, nicht die Mail: zwei Mails derselben
...
function csvCell(s: string): string {
	...
	// hier stammen aus fremden Mails – ein Absendername wie "=1+1" würde in der
	// Tabelle des Chefs ausgeführt.
