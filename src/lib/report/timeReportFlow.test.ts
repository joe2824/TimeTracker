// Durchstich über die ganze Kette, gegen die echte Datei:
// XLSX lesen -> Report auswerten -> abgleichen -> Nachtrag planen -> Einträge
...
import {
	...
	splitBlocks,
	type Share
} from "../time/timeReconcile";
import { fmtDate, noonTs, startOfNextDay, toTs } from "../time/time";
...
/** Einen Monat vollständig nachtragen und das Ergebnis zurückgeben. */
function fillMonth(month: string, deductBreaks: boolean, shares?: Share[]) {
	...
	for (const day of before.days) {
		...
		if (plan.kind === "absence") {
			...
			// Wie die Oberfläche: gestempelte Zeit auf das Projekt, die Stunden
			// jenseits der Stempel auf „Others". Mit Verteilung wird die gestempelte
			// Zeit stattdessen in Blöcke je Projekt geschnitten.
...
describe.each([true, false])("Durchstich mit Pausenabzug=%s", (deductBreaks) => {
	it("laesst nur Tage offen, an denen LOGA anders abzieht als die Hausregel", () => {
		for (const month of months) {
			...
			// Vorher gab es wirklich etwas zu tun – sonst prüft der Test nichts.
			...
			// Mit Abzug bleibt ein Rest, wo LOGAs tatsächlicher Abzug von der
			// Hausregel abweicht – im echten Report rund 11 % der Tage (gestempelte
			// Zusatzpausen, Korrekturbuchungen). Diese Tage sind nicht füllbar,
	...
	it("schlaegt beim zweiten Durchlauf nichts mehr vor", () => {
		// Sonst entstünden bei jedem Öffnen des Abgleichs neue Einträge.
		for (const month of months) {
	...
	it("legt keine sich ueberschneidenden Eintraege an", () => {
		// addEntry wiese solche Einträge ab – bei einer Sammelübernahme
...
/** Kein Eintrag eines Tages fängt an, bevor der vorige zu Ende ist. */
...
describe("Nachtrag und Stempelzeiten", () => {
	it("bleibt an gestempelten Tagen zwischen Kommen und Gehen", () => {
		for (const month of months) {
			...
			for (const e of entries) {
				...
				// Nachtrag über „Letztes gehen" hinausreichen.
	...
	it("spart die Pause nur ohne eigenen Abzug als Luecke aus", () => {
		...
		// ohne Abzug entsteht die Mittagslücke (zwei Blöcke).
...
// beim Schneiden darf keine Minute verloren gehen und keine doppelt zählen.
describe("Verteilung auf mehrere Projekte", () => {
	...
	it("bucht dieselbe Zeit, nur auf zwei Projekte statt eines", () => {
		for (const month of months) {
			...
			// „Others" bleibt unberührt: die ungestempelten Stunden werden nicht verteilt.
	...
	it("trifft das Verhaeltnis ueber den Monat", () => {
		for (const month of months) {
			...
			// Pro Tag auf die Minute gerundet – über einen Monat bleibt davon wenig.
	...
	it("verteilt tageweise dieselben Stunden, nur ohne Schnitt im Tag", () => {
		for (const month of months) {
			...
			// Wie die Oberfläche im Modus „tageweise": ein Projekt je Tag.
