...
// Mit ausgeschriebenen Parametern: sonst hält TypeScript die Aufrufliste des
// Mocks für leer, und jeder Zugriff auf ein Argument ist ein Typfehler.
...
/** Desktop vortäuschen: `capabilities.outlook` hängt an __TAURI_INTERNALS__. */
...
describe("sendReport", () => {
	...
	it("laedt den Monat nach, wenn er noch nicht im Speicher steht", async () => {
		// Der Bericht lässt sich für einen Monat auslösen, den die App in dieser
		// Sitzung nie geöffnet hat. Ohne das Nachladen stünde dort eine leere
		// Tabelle – ein Bericht über einen Monat ohne Arbeit.
...
describe("sendReport ohne Outlook", () => {
	...
	// Tabelle muss also über die Zwischenablage kommen.
	beforeEach(() => {
		pretendDesktop(false);
		app.entriesByMonth["2026-07"] = [entry("e1", at(16, 9), at(16, 12))];
	});
