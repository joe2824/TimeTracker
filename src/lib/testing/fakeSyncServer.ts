// Der nachgebaute Abgleich-Server für die Tests in src/lib/sync.
//
// Er kann genau das, was der Client anfasst: Datensätze annehmen (mit
// Konflikt), sie nach Stand und Zeitraum wieder herausgeben, die belegten
// Monate nennen und Auskunft über das Konto geben. Dazu Zählwerk für
// Aussagen über den Datenverkehr und ein Tor, mit dem ein Test einen
// laufenden Abruf festhalten kann.
import type { ServerRecord } from "../sync/api";

/** Welche Abrufe das Tor anhält. */
...
export class FakeSyncServer {
	...
	calls: string[] = [];
	/** Dasselbe mit Suchteil - für Aussagen über die Einschränkung. */
	queries: string[] = [];
	...
	/** Wie viele Datensätze je Art liegen. */
	...
	async #waitAtGate(filtered: boolean, unbucketed: boolean): Promise<void> {
		...
		// Menge trägt zusätzlich "unbucketed" und läuft weiter.
		const hit =
			gate.scope === "all" ||
