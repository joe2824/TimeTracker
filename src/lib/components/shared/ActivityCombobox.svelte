<script lang="ts">
	...
	function place() {
		...
		// Vor der ersten Messung die Maximalhöhe annehmen (max-h-52 = 208px).
		const height = list?.offsetHeight || 208;
		const below = window.innerHeight - r.bottom - 8;
		// Nur umklappen, wenn oben tatsächlich mehr Platz ist – sonst wäre die
	...
	$effect(() => {
		...
		// Die Liste wird beim Tippen kürzer: dann neu messen.
		filtered.length;
		place();
		const onMove = () => place();
		window.addEventListener("resize", onMove);
		// capture: auch das Scrollen innerhalb von Containern zählt.
