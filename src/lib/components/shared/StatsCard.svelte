<script lang="ts">
	...
	interface Props {
		/** ausgewählter Monat "YYYY-MM" */
		month: string;
		/** Bericht des Monats – liefert die bereits gerundeten Stunden je Aktivität */
		report: MonthReport;
	}
	...
	// Die Heatmap braucht das ganze Jahr, der Monatswechsel lädt aber nur einen Monat.
	$effect(() => {
	...
	// ihn (seine Dauer wächst); sonst liefe die komplette Jahresauswertung – zwei
	// Maps, das Wochenraster und ~370 keyed Spans – jede Sekunde neu. Abgeschlossene
	// Einträge haben ein endTs und lesen `now` gar nicht, der Wert ist dann egal.
	const runningInYear = $derived(
		app.running !== null && zonedParts(app.running.startTs).year === year
	);
	// Auf fünf Minuten gerundet: `app.now` tickt im Sekundentakt, ein Jahresraster
	// ändert sich dadurch aber nicht sichtbar – eine Zelle deckt einen ganzen Tag
	// ab, ihre Färbung hängt an Quartilen des Jahresmaximums. Ohne die Rundung
	// laufen Aufschlüsselung, Summen, Raster und ~370 keyed Spans jede Sekunde
	// neu, nur weil irgendwo ein Timer läuft.
	...
	// EIN Tooltip-Element für alle Zellen: ein Jahr hat ~365 davon, eine
	// Floating-Instanz je Zelle wäre reine Verschwendung. Gemerkt wird nur das
	// Datum – ein festgehaltener Tag wäre eine Kopie und veraltete, sobald ein
	// Timer an diesem Tag weiterläuft.
	 ...
	 * kein Platz mehr ist – die Heatmap steht am Seitenende, dort wäre er sonst
	 * abgeschnitten. Braucht die gemessene Grösse, deshalb kein reines CSS.
	 */
	...
	// ---- Saldo ----
	// Ist = Arbeitszeit + Abwesenheiten: ein Urlaubstag ist erfüllte Zeit, kein Minus.
	// Der Zeitausgleich zählt dabei mit, wie jede andere Abwesenheit.
	...
	// ---- Balken: Stunden je Aktivität ----
	// Nur Projektzeiten, absteigend – die Länge trägt die Aussage, nicht die Farbe.
	const bars = $derived(
		report.rows
	...
	// Eine Hue: der Job ist Grössenvergleich, nicht Identität – die Namen an der
	// Achse tragen die Identität, deshalb wären acht Farben hier verschenkt.
	const chartConfig = {
	...
	/**
	 * Monatswechsel als Spaltenindex – für die Beschriftung über dem Raster.
	 ...
	 */
	/** Gemessene Breite des Diagramms – entscheidet über die Beschriftungsspalte. */
	let chartWidth = $state(0);

	/**
	 * Platz für die Aktivitätsnamen links. Auf einer schmalen Karte kostet die
	 * feste Spalte ein Drittel der Fläche – dann lieber kürzere Namen als
...
<Card.Root>
	...
	<Card.Content class="space-y-8">
		...
		<div class="space-y-2">
			...
			{:else}
				<!-- Feste Zeilenhöhe statt Seitenverhältnis: sonst werden die Balken bei
				     wenigen Aktivitäten fett. Als Style, weil Tailwind dynamische
				...
				<!-- Tabelle als zweiter Kanal: exakte Werte, unabhängig von der Farbe. -->
		...
		<div class="space-y-2">
			...
			{#if !yearReady}
				<!-- Platzhalter im Zuschnitt des Wochenrasters: sonst füllt sich die
				     Heatmap sichtbar von hinten nach vorn, während die Monate
				     nachladen. -->
...
{#if hover}
	<!-- Folgt dem Zeiger; kippt an Viewport-Rändern (siehe tipPos). -->
	<div
...
<style>
	/* Seqünzielle Rampe (eine Hue, hell->dunkel) – in BEIDEN Themes in dieselbe
	   ...
	   statt im Schwarz – dunkel genug für die Aussage, hell genug gegen die
