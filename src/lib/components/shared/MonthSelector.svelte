<script lang="ts">
	...
	interface Props {
		...
		month: string;
		/** id des Triggers, damit das Label korrekt verknüpft ist */
		id?: string;
	}
	let { month = $bindable(), id = "month" }: Props = $props();

	/** Monate mit Einträgen (von der Platte). */
	let stored = $state<string[]>([]);

	// Neu lesen, sobald sich Einträge irgendwo geändert haben - sonst bliebe ein
	// gelöschtes Jahr bis zum Neuladen der Seite in der Auswahl.
	$effect(() => {
		app.entriesVersion;
	...
	// Nur solange die Historie noch läuft. Danach steht alles auf der Platte -
	// dann wäre die Anfrage bei jeder Ansicht mit Monatsauswahl für nichts.
	$effect(() => {
	...
	// Liste schreiben – er verschwindet, sobald man ihn verlässt. Aktueller und
	// ausgewählter Monat sind immer dabei, sonst hat der Trigger nichts anzuzeigen.
	 ...
	 * Verbindung nach "nichts passiert" aus - dabei läuft der Abruf gerade.
	 */
	...
	<div class="flex items-center gap-2">
		<ButtonGroup.Root>
			...
			<Select.Root type="single" bind:value={month}>
				<!-- Feste Breite: der längste Monatsname ("September 2026") passt hinein,
				     dadurch springt der Pfeil rechts beim Blättern nicht. -->
