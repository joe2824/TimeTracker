<script lang="ts">
	...
	/** Bis hierhin gilt der Stand als frisch - dann genügt der Punkt. */
	...
	interface Status {
		...
		text: string;
		titleText: string;
		/** Läuft gerade etwas? Dann dreht sich das Icon statt des Punktes. */
		busy?: boolean;
	}
	...
	const statusInfo = $derived.by((): Status | null => {
		...
		// Die Historie läuft nebenher weiter. Sie steht hier, weil daran die
		// gesperrte Sicherung hängt - aber leise, es wartet niemand darauf.
...
<!--
	...
	frisch) -> "Daten werden geladen …" -> "". Ohne den festen Platz ändert sich
	dabei die Breite dieser Zeile, und weil die Kopfzeile rechtsbündig steht,
