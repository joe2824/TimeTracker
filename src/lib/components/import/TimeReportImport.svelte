<script lang="ts">
	...
	/** Die eingelesene Datei, solange die Auswahl von Person/Monat läuft. */
	...
	/** Der auf Platte liegende Report des aktuellen Monats (für den Ruhezustand). */
	...
	// Auch am Abgleich hängen: ein Report von einem anderen Gerät kommt herein,
	// während diese Karte offen steht.
	$effect(() => {
		void app.entriesVersion;
		void refreshReportMonths();
	});

	/** Auswahl und Zuordnung je Tag, Schlüssel = Datum. */
	let selected = $state<Record<string, boolean>>({});
	let activityFor = $state<Record<string, string>>({});
	/** Zuordnung der Stunden JENSEITS der Stempelzeiten, Schlüssel = Datum. */
	let extraActivityFor = $state<Record<string, string>>({});
	/** Tage, die auf MEHRERE Projekte aufgeteilt werden, Schlüssel = Datum. */
	...
	// Im Ruhezustand zeigen, ob für diesen Monat schon ein Report vorliegt.
	$effect(() => {
		const m = month;
		void app.entriesVersion;
	...
	/** Bilanz des gespeicherten Reports für die Ruhe-Ansicht der Karte. */
	...
	/** Wem die eingelesenen Reports gehören – der Name aus "Bericht & E-Mail". */
	...
	/** Auswahl und Aktivitäten für den aktuellen Abgleich neu setzen. */
	...
	/** Die ausgewählten Tage mit Zeitnachtrag – nur die lassen sich verteilen. */
	...
	/** Die Anteile auf die ausgewählten Tage legen. */
	...
	async function handleFile(file: File) {
		...
		try {
			...
			parsed = report;
			// Auch bei genau einer Person wird geprüft, ob es die eigene ist: die
			// Anzeige weist den Report ab hier mit dem eigenen Namen aus, und
			// abgeglichen würde gegen die eigenen Zeiten. Bei einem fremden Namen
			// wird gefragt statt gemeldet - eine Meldung übersieht man.
	 ...
	 * die Datei ihn enthält, sonst der letzte darin.
	 */
	...
	/**
	 * Die eigene Person in einem Team-Export raten: über den Absendernamen aus
	...
	/**
	 * Gehört diese Person zu diesem Konto?
	 *
	 * Ohne eigenen Namen in den Einstellungen laesst sich nichts vergleichen -
	 * dann wird übernommen wie bisher, und die Anzeige nennt mangels Namen auch
	 * niemanden.
	 */
	...
	async function useReport(key: string, targetMonth: string) {
		...
		// Nichts speichern, was nichts enthält: ein leerer Monat würde einen
		// früher eingelesenen Report desselben Monats überschreiben, und der
	...
	async function switchMonth(target: string) {
		...
		// `parsed` bleibt stehen: sonst verschwänden die übrigen Monate der noch
		// offenen Datei aus der Auswahl und wären nur über einen neuen Import
		// wieder zu erreichen.
		active = report;
		stored = report;
		resetSelection();
	...
	/** Den gespeicherten Report dieses Monats wieder öffnen. */
		...
		try {
			for (const { day, plan } of chosen) {
				...
				if (plan.kind === "absence") {
					...
					// Gestempelte Zeit und die Stunden jenseits davon können auf
					// verschiedene Aktivitäten gehen; die gestempelte Zeit selbst
					// zusätzlich auf mehrere, wenn der Tag verteilt wurde.
					const inside =
						split.length > 0
	...
	{:else if stage === "idle"}
		...
		<Card.Content class="flex flex-1 flex-col gap-3">
			<!-- Ablegen UND Klicken auf demselben Element: ein <button> bringt Fokus
			     und Tastaturbedienung mit, ein <div> mit Drop-Handler hätte beides nicht. -->
			<button
				type="button"
				...
				<div class="flex flex-wrap items-center justify-between gap-3">
					<div class="flex flex-wrap items-center gap-2">
						{#if availableMonths.length > 1}
							...
							<!-- Pfeile wie in der Monatsauswahl der Einträge-Ansicht, hier aber
