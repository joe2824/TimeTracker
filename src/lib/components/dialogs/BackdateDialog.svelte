<script lang="ts">
	// Rückfrage vor einem rückdatierten Timer-Start, der bereits erfasste Zeiten
	// anfasst.
	...
	interface Props {
		/** Nach dem tatsächlichen Start – das Tray meldet damit dem Hauptfenster. */
		onapplied?: () => void;
	}
	...
	/** Nur abgeschlossene Zeiten sind der Grund für die Rückfrage. */
	const rows = $derived<Row[]>([
		...
		// Auch laufende Einträge zeigen: die verschwinden sonst kommentarlos.
		...(p?.plan.remove ?? []).map((e) => ({
			id: e.id,
