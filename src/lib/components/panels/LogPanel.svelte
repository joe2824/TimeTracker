<script lang="ts">
	...
	/** Nur Warnungen und Fehler – der übliche Blick bei „irgendwas stimmt nicht". */
	let onlyProblems = $state(false);

	const shown = $derived(onlyProblems ? problems(lines) : lines);

	/**
	 * Warnungen und Fehler – mitsamt ihren eingerückten Fortsetzungszeilen.
	 * Ein Fehler ohne seine Aufrufliste ist genau die Hälfte, die nicht hilft.
	 */
	...
	/** Lesen, sobald die Karte tatsächlich zu sehen ist. */
	$effect(() => {
	...
	async function openFolder() {
		try {
			// Auf die heutige Datei zeigen: der Explorer öffnet dann den Ordner und
