<script lang="ts">
	...
	async function saveTracking() {
		...
		toast.success("Tracking-Einstellungen gespeichert.");
	}
	...
	async function saveGeneral() {
		await app.updateSettings({
			...
			reportSubjectTemplate: subjectTpl.trim() || "Stundenerfassung {month}"
		});
...
<div class="grid gap-4 lg:grid-cols-2">
	<Card.Root>
		...
		<Card.Content class="space-y-3">
			...
			<div class="space-y-1">
				...
				<p class="text-muted-foreground text-xs">{"{month}"} wird durch den Monat ersetzt.</p>
			</div>
	...
	<Card.Root>
		...
		<Card.Content class="space-y-4">
			...
			<Button size="sm" onclick={saveTracking}>Tracking speichern</Button>
		</Card.Content>
	</Card.Root>
</div>
