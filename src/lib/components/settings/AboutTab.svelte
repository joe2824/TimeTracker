		...
		{#if app.devMode}
			<div>
				...
				<div class="flex flex-wrap gap-2">
					...
					{#if !isTauri()}
						<!-- Das Web-Onboarding haengt an `needsLogin` und war sonst nur
						     erreichbar, solange das Konto nicht verknuepft ist. Je Schritt
						     ein Knopf: ohne den zweiten landet man immer am Anfang. -->
						<Button
							variant="secondary"
							size="sm"
							onclick={() => ((onboardingOpen.step = "start"), (onboardingOpen.value = true))}
						>
							Web-Onboarding: Start
						</Button>
						<Button
							variant="secondary"
							size="sm"
							onclick={() => ((onboardingOpen.step = "device"), (onboardingOpen.value = true))}
						>
							Web-Onboarding: Gerät koppeln
						</Button>
						<Button
							variant="secondary"
							size="sm"
							onclick={() => ((onboardingOpen.step = "phrase"), (onboardingOpen.value = true))}
						>
							Web-Onboarding: 24 Wörter
						</Button>
					{/if}
