<script lang="ts">
	...
	const report = $derived(
		buildReport(
			...
			// Bewusst Date.now() statt app.now: sonst hänge die Vorschau am
	...
	async function sendToOutlook(): Promise<boolean> {
		...
		try {
			...
			// Klar erklären (z.B. nur neues Outlook) und den mailto-Fallback anbieten.
	...
	async function prepareMail(): Promise<boolean> {
		...
		// Die Anleitung selbst steht im Entwurf - hier reicht die Rückmeldung,
...
<div class="space-y-4">
	<div class="flex flex-wrap items-end justify-between gap-3">
		...
		<div class="flex flex-wrap gap-2">
			...
			<!--
				Ohne Outlook bleibt der Weg über das Mailprogramm des Systems - den
				gab es ohnehin schon als Rückfall, wenn Outlook nicht mitspielt.
			-->
	...
	<Card.Root>
		...
		<Card.Content class="space-y-3">
			<!-- Vier Zahlen, die man gegen den Zeitnachweis hält. Gesamt ist die Zahl,
			     um die es geht, und trägt deshalb als einzige Farbe. -->
			<div class="grid grid-cols-2 gap-3 {report.breakHours > 0 ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}">
				...
				{#if report.breakHours > 0}
					<!-- Der Abzug steckt bereits in den Zahlen der Tabelle. Er gehört
...
<Dialog.Root bind:open={previewOpen}>
	<!-- sm:-Stufe nötig: dialog-content setzt selbst sm:max-w-sm, ein blankes
	     max-w-* würde davon überschrieben. Die Tabelle ist 520px breit. -->
	<Dialog.Content class="sm:max-w-3xl">
		...
		<!-- Feste helle Fläche: die Tabelle trägt Inline-Styles für Outlook und
		     würde im Dunkelmodus sonst schwarz auf dunkel stehen. -->
		<div class="max-h-[60vh] overflow-auto rounded-md border bg-white p-4 text-black">
			<!-- Mittig über `mx-auto` im Block-Layout, nicht per Flex: die Tabelle ist
			     520px breit, im Scroll-Container würde flex+center bei wenig Platz den
			     linken Rand abschneiden. Auto-Margins fallen beim Überlaufen auf 0. -->
