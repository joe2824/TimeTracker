<script lang="ts">
	...
	/**
	 * Gefundene Mails, die einen anderen Monat betreffen. Der Filter greift über
	 * den Betreff, der Zeitraum über das Empfangsdatum – wer seinen Januar-Bericht
	 * im März nachreicht, taucht deshalb im Posteingang auf, aber nicht in dieser
	...
	// Ein Ergebnis gilt für den Monat, für den es geholt wurde. Ohne dieses
	// Zurücksetzen stünde nach dem Umschalten die Ausbeute des Vormonats unter
	// der neuen Monatsüberschrift – gefiltert, also plausibel leer, aber falsch.
	$effect(() => {
					...
					<div class="grid grid-cols-2 gap-3 px-4 {otherMonth > 0 ? 'sm:grid-cols-3' : ''}">
						...
						{#if otherMonth > 0}
							<!-- Mails, die der Filter fand, die aber einen anderen Monat betreffen.
							     Gehört genannt: sonst wundert man sich, warum der Posteingang mehr
