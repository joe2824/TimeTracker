<script lang="ts">
	...
	//
	// Der Startschritt ist zugleich die öffentliche Seite - Landing trägt sie,
	...
	// Ein gesetzter Schritt kommt vom Dev-Knopf. Einmalig lesen und zuruecksetzen:
	// sonst spraenge die Ansicht bei jedem weiteren Rendern wieder dorthin.
	$effect(() => {
		if (!onboardingOpen.step) return;
		// Die Phrase entsteht sonst erst beim Registrieren; ohne sie zeigte der
		// Schritt 24 leere Kacheln. Nur fuellen, wenn nichts dasteht.
		if (onboardingOpen.step === "phrase" && !phrase) phrase = createRecoveryPhrase();
		stepIndex = onboardingOpen.step;
		onboardingOpen.step = "";
	});
	...
	/** Zu welchem Konto der eben geholte Schlüssel gehört. */
	let loggedInId = "";
	 ...
	 *
	 * WebAuthn will unmittelbar auf die Berührung folgen; bei langsamer
	 * Verbindung ist die Berechtigung aus dem Klick sonst abgelaufen, bevor der
	 * Dialog aufgeht. Fehlschläge bleiben still - der Klick fragt dann eben
	 * selbst, wie bisher.
	 */
	function warmLogin() {
	...
	// Nicht erst beim Zeigen: auf dem Touchscreen liegen Berührung und Klick so
	// dicht beieinander, dass die Anfrage sonst noch läuft, wenn der Klick kommt
	...
	async function signIn() {
		...
		try {
			...
			if (await account.unlockWithStoredKey(serverUrl, r.userId)) {
				...
				// aufgebaut ist, und fände sonst den Stand von vorher vor.
				await account
			...
			// aufgemacht - die Berechtigung aus der Berührung war abgelaufen,
			// während die Aufgabe noch unterwegs war. Ein zweiter Versuch geht
	...
	/**
	 * Begrüsst wird mit dem Namen aus "Bericht & E-Mail" - der steht hier auf dem
	 * Gerät und ist der, den die Person selbst gesetzt hat. Ist er leer, bleibt
	...
	/** Einmal ermitteln - das ändert sich während einer Sitzung nicht. */
	const os = detectOs();

	// Was der Link mitgebracht hat. Wird dabei aus der Adresszeile entfernt - alles
	// davon gilt genau einmal und nützt in der Chronik niemandem.
	 ...
	 *
	 * Ob einer nötig ist, weiss nur der Server, und die meisten Installationen
	 * brauchen keinen. Ein Feld, das fast immer leer bleibt, steht sonst als
	 * erstes im Weg. Verlangt der Server einen, kommt er mit 403 zurück - dann
	...
	async function applyPhrase() {
		if (!keyValue) return;
		// VOR dem Verknüpfen: in dem Moment, in dem das Konto hängt, baut die
	...
	/** Den Code aus der Desktop-Anwendung bestätigen. */
	...
	// ---------- Der Browser koppelt sich wie ein neues Gerät ----------

	/** Steht der Kopplungs-Dialog offen? */
	let pairingOpen = $state(false);

	// Holt den Code und sieht danach selbst alle zwei Sekunden nach, ob drüben
	// bestätigt wurde - dieselbe Klasse wie in PairStartDialog.svelte, statt
	// Code holen/Takt/Abbrechen hier ein zweites Mal nachzubauen.
	...
	/** Läuft eine Kopplung, die auf die Bestätigung in der App wartet? */
	let waitingForApp = $state(false);
	/** Der Code als Rückfalltür - eingeklappt, bis jemand ihn braucht. */
	let showAppCode = $state(false);

	/**
	 * Die Anwendung bitten, selbst eine Kopplung zu beginnen.
	 *
	 * Hier haengt das Konto, drueben nicht - also erzeugt DIE App den Code: er ist
	 * der Abdruck ihres Geraeteschluessels, den wir gleich mit dem Vault-Schluessel
	 * versorgen. Der Link traegt deshalb nur die Serveradresse.
	 *
	 * Der Code kommt bewusst nicht von allein zurueck. Ihn selbst herueberzutragen
	 * ist die Pruefung, dass wirklich diese Anwendung ankommt.
	 */
	async function pairViaApp() {
		running = true;
		try {
			waitingForApp = true;
	...
	// Verschwindet der Bildschirm, waehrend ein Code offen ist (z.B. Navigation
	// mitten in der Kopplung), soll der Zwei-Sekunden-Takt nicht weiterlaufen.
...
{#if stepIndex === "start"}
	<!-- Der Startschritt IST die Landing Page: Werbung und Anmeldung auf einer
	     Seite. Landing trägt nur die Seite, die Karte kommt als Snippet von
	     hier - der Vault-Schlüssel hat in einer Marketingkomponente nichts zu
	     suchen. -->
	<Landing {os}>
		{#snippet auth()}
			<Card.Root class="w-full shadow-md border-border/80 bg-card">
				...
				<Card.Content class="space-y-4 pt-4">
					{#if linkValue.fresh}
						<!-- Der Rechner hat hierher geschickt, ausdrücklich zum Anlegen. -->
		...
		{#if stepIndex === "device"}
			...
			<Card.Root class="w-full">
				<Card.Content class="space-y-5 pt-6">
					<div class="space-y-2">
						<div>
							<p class="text-sm font-medium">Zeiten auch auf dem PC erfassen?</p>
							<p class="text-muted-foreground text-xs mt-0.5">
								Die App läuft im Hintergrund, mit Symbol in der Leiste und Tastenkürzeln.
								Verbunden zeigt sie dieselben Zeiten wie hier.
							</p>
						</div>

						<!-- EIN Feld, nicht zwei: der Code ist derselbe, egal ob die Anwendung
						     hier liegt oder auf einem anderen Rechner. Der Knopf darueber nimmt
						     nur den Weg dorthin ab. -->
						{#if !waitingForApp}
							<Button variant="outline" class="w-full" disabled={running} onclick={pairViaApp}>
								{running ? "Öffnet…" : "App auf diesem PC öffnen"}
							</Button>
							<!-- Wer hier merkt, dass die Anwendung noch fehlt, soll sie nicht
							     erst suchen muessen. -->
						...
						<Label for="appcode" class="text-muted-foreground text-xs font-normal">
							{#if waitingForApp}
								Die App zeigt jetzt einen Code – trag ihn hier ein.
							{:else}
								Oder den Code aus der App eintragen. Sie zeigt ihn unter Einstellungen →
								Konto.
							{/if}
						</Label>
						...
						{#if waitingForApp && !showAppCode}
							<Button
								variant="ghost"
								size="sm"
								class="h-7 px-2 text-xs"
								onclick={() => (showAppCode = true)}
							>
								Nichts passiert?
							</Button>
						{:else if waitingForApp}
							<p class="text-muted-foreground text-xs">
								Dann öffne die App von Hand: Einstellungen → Konto →
...
>
	<!-- max-w-md, nicht -sm: der Code ist 14 Zeichen weit gesperrt und braucht
	     mitsamt Innenabständen 422 px. Bei 384 px brach er nach "PT8F-" um, und
...
<Dialog.Root
	...
	onOpenChange={(o) => {
		...
		// Wörter beim nächsten Öffnen wieder sichtbar im Feld.
		if (!o) phraseInput = "";
	}}
>
	<Dialog.Content class="sm:max-w-md">
