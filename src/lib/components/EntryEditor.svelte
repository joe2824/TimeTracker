<script lang="ts">
	...
	import CalendarImport from "$lib/components/import/CalendarImport.svelte";
	...
	// dito für den Abgleich mit dem Zeitwirtschaftsreport.
	...
	// Von/Bis sind Uhrzeiten eines Tages; läuft Bis vor Von, gilt der Folgetag.
	// Damit ist die längste darstellbare Spanne knapp unter 24 h – bei genau 24 h
	// wäre das Ende wieder der Start, also ein 0-Stunden-Eintrag.
	...
	const dayAdjust = $derived.by(() => {
		...
		for (const e of app.monthEntries(draft.date.slice(0, 7))) {
			...
			// erfasste Zeit, und als "abwesend" gerechnet zöge er das Ziel dieses
			// Eintrags fälschlich gegen null.
			if (app.isTimeOff(e)) continue;
			const isAbs = app.isAbsenceId(e.activityId);
			// Gekappt wie in `reconcile`: ein vergessener offener Eintrag zählte sonst
	 ...
	 *
	 * Die Liste zeigt jeden Kalendertag, auch ohne Einträge - ein Monat, der
	 * gerade erst geholt wird, sähe deshalb aus wie ein leerer. Statt dessen
	 ...
	 * ebenfalls auf dieser Höhe, obwohl dort niemand gescrollt hat.
	 * Über die Rechtecke statt `offsetTop`, weil weder Liste noch Zeile
	...
	function centerRow(date: string): boolean {
		...
		// nächsten Frame erneut versuchen.
	...
	async function jumpToDate(date: string) {
		...
		// Steht der Monat noch aus, gibt es keine Zeilen - der nächste Durchlauf
		...
		// Durchläufen - ein einzelnes `tick()` reicht nicht.
		for (let i = 0; i < 10; i++) {
	...
	// noch gar nicht, und der Wunsch wäre verbraucht.
	$effect(() => {
	...
	async function save() {
		...
		// Von == Bis wäre ein 0-Stunden-Eintrag: erfasst nichts, steht aber im Weg.
...
<div class="space-y-4">
	{#if !anyPreview}
	<!-- Die Monatssumme stand als nackter Σ-Span zwischen den Knöpfen und sah aus
	     wie eine Beschriftung, die zum nächsten Knopf gehört. Sie gehört zum
	...
	<Card.Root>
		<Card.Content class="p-0">
			<!-- Kein divide-y: die Linie UNTER einer Zeile wäre der border-top der
			     nächsten, damit liesse sie sich für heute nicht abschalten. Mit
			...
			<!-- Eigener Scroll-Bereich auf jeder Breite: der Sprung auf einen Tag soll
			     ...
			     auch alle anderen Tabs auf dieser Höhe. dvh statt vh, weil die
			...
			>
				{#each days as day, i (day.date)}
					...
					<!-- Heute trägt nur den linken Balken: die Zeilen konkurrieren schon mit
					     Amber (Abwesenheit) und bg-muted (freier Tag). Die Trennlinien ober-
					     und unterhalb entfallen, damit der Block zusammenhängt – deshalb auch
						...
						<div class="ml-auto flex items-center gap-2 pt-0.5 sm:ml-0">
							...
							{#if day.pause > 0}
								<!-- Der Abzug muss sichtbar sein: die Tagessumme daneben ist sonst
								     unerklärlich niedriger als die Einträge darüber. -->
								<Popover.Root>
									<Popover.Trigger
	...
	<!-- Nebeneinander, damit beide Wege sichtbar sind: untereinander rutschte der
	     ...
	     Übernimmt eine Karte die Ansicht, weicht die andere und die verbleibende
	     bekommt die volle Breite. -->
	<!-- Gleiche Höhe für beide Karten (kein items-start): unterschiedlich hohe
	     Nachbarn sahen unfertig aus. Die kürzere Karte setzt ihre Aktion dafür
		...
		<form class="grid gap-4" onsubmit={(e) => { e.preventDefault(); save(); }}>
			...
			{#if draftIsAbsence}
				...
				<!-- Der Unterschied steckt allein in der Verrechnung, nicht in der
				     Erfassung: deshalb hier eine Auswahl statt einer eigenen Aktivität. -->
				<AbsenceKindChoice id="timeoff" bind:value={draft.timeOff} />
				<DayFractionSwitch id="frac" bind:value={draft.fraction} />
			{:else}
				<!-- Schmal zwei Spalten: Von/Bis nebeneinander, Stunden über die volle
