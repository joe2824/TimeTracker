<script lang="ts">
	...
	// tickt im Sekundentakt, die formatierte Fassung ändert sich nur um
	// Mitternacht, und einen unveränderten Wert gibt Svelte nicht weiter – die
	// Rechnung läuft also einmal am Tag und überlebt trotzdem den Tageswechsel.
	...
	const arbzgVerdict = $derived.by(() => {
		...
		// Die Card darf über eine kurze Datenbasis rechnen, weil sie sie
		...
		// erfassten Monat würde sonst eine Warnung, die nur heisst, dass die
		// übrigen fünf fehlen.
		const w = r.windows.strict;
		if (!w.complete && w.weeksCovered < MIN_HINT_WEEKS) return null;
		const v = r.forecasts.strict.verdict;
		// Nur wenn wirklich etwas zu tun ist – also wenn man spürbar herunter
		// müsste, um das Fenster zu halten. "Dicht an der Grenze" bleibt der Card
		// vorbehalten: eine Dauerwarnung auf der meistgesehenen Seite liest nach
		// einer Woche niemand mehr, und dann fällt auch die nicht mehr auf, die
		// etwas von einem will.
		return v.requiresAction ? v : null;
	});

	/**
	 * Form und Tönung der Hinweiszeile.
	 *
	 * Gedeckt gehalten: der Hinweis steht über dem Timer und darf ihn nicht
	 * übertönen. Dieselben Stufen wie das Urteil im Arbeitszeit-Check, damit
...
<div class="space-y-4">
	...
	<!--
		Der laufende Timer ist die Hauptaussage der Seite und trägt deshalb Farbe,
		solange er läuft – vorher war er eine Karte wie jede andere, und ob
		überhaupt etwas lief, musste man lesen statt sehen. Steht er, bleibt die
	...
	<div>
		...
		{:else}
			<!-- Eine Spalte erst ab ~420px verdoppeln: darunter bleibt neben Symbol,
			     Punkt und Stern kaum Platz für den Namen. -->
		...
		<Card.Header class="flex flex-row flex-wrap items-start justify-between gap-2 space-y-0">
			<div class="min-w-0">
				...
				{#if todaySum.worked > 0 || todaySum.absent > 0 || todaySum.timeOff > 0}
					<!-- Kleine Tagesbilanz: erfasst, Abzug, Arbeitszeit. Ohne sie müsste
					     man die Zeilen darunter im Kopf zusammenrechnen – und der
					     Pausenabzug wäre gar nicht zu sehen. -->
