 ...
 * Installation) gehört dem Plugin. Geprüft wird hier, was die App daraus macht:
...
/** Ein gefundenes Update, dessen Installation die übergebenen Ereignisse meldet. */
...
beforeEach(async () => {
	// `silentFailureLogged` lebt am Modul und trägt aus dem vorigen Test herüber.
	// Zurückgesetzt wird er nur von einer gelungenen Suche – also einmal eine
	// laufen lassen, sonst hängt jeder Test daran, was der vorige getan hat.
...
describe("checkForUpdate", () => {
	...
	it("schreibt einen wiederkehrenden Ausfall im Hintergrund nur einmal ins Protokoll", async () => {
		// Die Suche läuft stündlich. Ohne diese Wache stünde eine Woche ohne Netz
		// als stündliche Warnung im Protokoll – und zwar in genau der Ansicht, die
...
describe("openUpdateDialog", () => {
	...
	it("sucht nach, wenn nichts gemerkt ist", async () => {
		// Der Fall nach einem reload() der Seite: der Knopf im Hinweis-Toast führte
		// sonst ins Leere.
		plugin.check.mockResolvedValue(update("0.9.1"));
