...
describe("warm", () => {
	...
	it("gibt das laufende Versprechen heraus, statt ein zweites zu starten", async () => {
		// Der Klick mitten im Laden soll sich anhängen - sonst käme die Antwort
		// zweimal über die Leitung.
	...
	it("brennt einen Fehlschlag nicht ein", async () => {
		// Sonst wiederholte der nächste Versuch nur den alten Fehler, statt zu fragen.
...
describe("onIntent", () => {
	...
	it("laedt nicht, wenn der Zeiger vorher weiterzieht", () => {
		// Eine Maus, die über die Tab-Leiste wischt, darf nicht jedes Ziel auslösen.
		const run = vi.fn();
		const h = onIntent(run, 100);
		h.onpointerenter();
