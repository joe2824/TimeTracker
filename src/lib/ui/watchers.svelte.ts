 ...
 * Versuch läuft sofort beim Start; dieser Takt holt nur nach, was daran
 ...
 * Gerät, dessen Meldung abgewiesen wurde, eine volle Stunde lang im Takt der
 * Prüfung gegen den Server.
 */
...
/** Läuft gerade eine Tagesmeldung? Der Tick kommt jede Sekunde wieder. */
let pinging = false;
/** Wann zuletzt versucht - unabhängig davon, ob es geklappt hat. */
 ...
 * Deckel klopfte ein solches Gerät den ganzen Tag weiter und brächte hinter
 ...
 * in zwanzig Minuten verbraucht und fällt aus der Zählung, obwohl er um 12
 * längst wieder online ist.
 */
let failedPingSlot = "";
let failedPingCount = 0;
/** So viele vergebliche Versuche je Stunde, dann Ruhe bis zur nächsten. */
const MAX_PING_FAILURES = 5;
/**
 * Der Server, der die Meldung abgelehnt hat - dort nicht mehr fragen. Ein
 * älterer Server kennt den Endpunkt nicht und antwortet dauerhaft mit 404;
 * ein Büro hinter einer gemeinsamen Adresse brächte damit nur die Bremse zum
 * Anschlagen.
 *
 * Die Adresse und nicht bloss ein Ja/Nein: wer sich danach mit einem anderen
 * Server verknüpft, soll dort wieder zählen dürfen.
 */
let declinedServer: string | null = null;

/**
 * Einmal je Kalendertag „aktiv" melden, sobald die App läuft. Ohne feste
 * Uhrzeit: wer nur zwischen zwei Terminen kurz aufmacht, fällt sonst aus der
 * Zählung.
 */
async function dailyPing(s: Settings): Promise<void> {
	if (pinging) return;
	// Ohne Ziel gibt es nichts zu melden - und nichts, was als Fehlversuch gelten
	// dürfte. Das Ziel ist der verknüpfte Server, sonst der aus dem Build.
	...
	try {
		...
		// Sonst fiele ein Gerät, das gerade offline oder ausgebremst war, für
		// diesen Tag ersatzlos aus der Zählung.
...
/**
 * Die Wächter, die es nur in der Desktop-Hülle gibt: Leerlauf, Auto-Stop,
