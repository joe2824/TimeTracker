...
/**
 * Ersatz für das global-shortcut-Plugin.
 *
 * Über `vi.hoisted`, weil vi.mock() an den Anfang der Datei wandert: eine
 * gewöhnliche Konstante wäre dort noch nicht angelegt.
 *
 * Der Fake weist ein doppelt vergebenes Kürzel ab – so verhält sich das echte
 * Plugin auch, und genau daran hängt die Frage, ob ein einzelnes untaugliches
 * Kürzel die übrigen mitreisst.
 */
const gs = vi.hoisted(() => {
...
describe("acceleratorFromEvent", () => {
	...
	it("liefert null, wenn nur Modifier gedrueckt wurden", () => {
		// Sonst entstünde beim Aufnehmen eines Kürzels schon ein Eintrag, sobald
		// jemand Strg drückt – bevor er die eigentliche Taste erreicht hat.
	...
	it("nimmt die physische Taste, nicht das erzeugte Zeichen", () => {
		...
		// amerikanischen; `code` ist auf beiden KeyL. Ein Kürzel darf nicht davon
		// abhängen, welches Layout gerade aktiv ist.
...
describe("applyShortcuts", () => {
	...
	it("raeumt vor jedem Durchgang auf", async () => {
		...
		// Das alte Kürzel darf nicht liegen bleiben: sonst startete eine gelöschte
		// oder umbelegte Aktivität weiter mit.
	...
	it("laesst das Umschalt-Kuerzel gewinnen, wenn eine Aktivitaet dasselbe traegt", async () => {
		...
		expect(gs.register).toHaveBeenCalledTimes(1);
		// Der Aufruf des Kürzels muss den Umschalter treffen, nicht den Start.
	...
	it("reisst die uebrigen Kuerzel nicht mit, wenn eines abgewiesen wird", async () => {
		// Der Fall aus dem Betrieb: ein Kürzel ist schon von einem anderen Programm
		// belegt, register() wirft. Ohne den Fang blieben alle danach folgenden
		// Aktivitäten ohne Kürzel – und zwar lautlos.
