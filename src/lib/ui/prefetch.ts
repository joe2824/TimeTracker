 ...
 * Klick mittendrin hängt sich daran an, statt eine zweite Anfrage zu starten.
 */
export function warm<T>(key: string, fn: () => Promise<T>, ttlMs = DEFAULT_TTL_MS): Promise<T> {
	...
	// Ein Fehlschlag darf sich nicht einbrennen: der nächste Versuch soll wieder
...
/** Alles verwerfen. Beim Kontowechsel: der Puffer gehört dem vorigen Konto. */
 ...
 * gebaut, ein Timer in ihrer Closure überlebte das Verlassen des Elements nicht.
 */
 ...
 *
 * Zeiger drauf, Tastaturfokus drauf oder Finger drauf - auf einem Touchgerät
 * gibt es kein Hover, dort ist die Berührung das früheste Signal.
 *
 * Als Handler und nicht als `use:`-Action, weil die Ziele hier Komponenten sind
 * (Button, Select.Item, Tabs.Trigger) und Actions nur an Elemente dürfen.
 *
 * ```svelte
...
export function onIntent(run: () => void, delay = DEFAULT_DELAY_MS) {
	const start = () => {
		// Entprellt: ohne das löst eine Maus, die über die Tab-Leiste wischt,
...
/** Puffer-Schlüssel des Kontos: Anmeldung, Geräte und Passkeys in einem Zug. */
export const ACCOUNT_KEY = "account";

/**
 * Konto, Geräte und Passkeys.
 *
 * Eine Anfrage für alle drei: `AccountInfo` bringt `devices` und `passkeys`
 * bereits mit.
 */
...
/** Einladungen und Sicherungen - nur, wenn jemand überhaupt Verwalter ist. */
...
export async function prefetchSettings(): Promise<void> {
	...
	// überhaupt zu holen gibt.
	await prefetchAccount();
	await prefetchAdmin();
}

/** Puffer-Schlüssel der Monatsliste vom Server. */
export const REMOTE_MONTHS_KEY = "months:remote";

/**
 * Welche Monate der Server kennt - für die Auswahl, solange der Backfill läuft.
 *
 * Länger gepuffert als der Rest: die Liste ändert sich nur, wenn irgendwo ein
 * neuer Monat entsteht, und sie hängt an einer Auswahl, die auf jeder Ansicht
 * steht.
 */
 ...
 *
 * Beide Schritte gehören zusammen - ohne den zweiten liegt der Monat zwar auf
