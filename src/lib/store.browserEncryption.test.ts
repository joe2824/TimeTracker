// Im Browser müssen Aktivitäten, Einstellungen, Einträge und Reports
// verschlüsselt auf der Ablage liegen - device.json und outbox.json nicht.
...
describe("lokale Verschlüsselung im Browser", () => {
	it("legt Aktivitäten als JWE ab, nicht als lesbares JSON", async () => {
		...
		// JWE-Compact: fünf durch Punkte getrennte Base64url-Abschnitte, kein JSON.
	...
	it("eine von Hand geschriebene Klartextdatei liest sich weiterhin korrekt und wird beim nächsten Speichern verschlüsselt", async () => {
		...
		// Nächstes Speichern verschlüsselt sie.
	...
	it("falscher Schlüssel öffnet nichts - und die Datei bleibt erhalten", async () => {
		// Der gefährliche Fall: Aktivitäten, Einstellungen und Report gäben ohne
		// Quarantäne den Ersatzwert zurück, und der nächste Speichervorgang
		// schriebe diese leere Sicht über die intakten Dateien - beim Abgleich bis
	...
	it("ohne Schlüssel bleibt die verschlüsselte Datei unangetastet liegen", async () => {
		// Kein Schlüssel heisst nicht beschaedigt - hier darf NICHTS umbenannt
		// werden, sonst räumte ein Start ohne Schlüssel den ganzen Bestand weg.
