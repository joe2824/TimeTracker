...
export interface StorageBackend {
	...
	/** Anhängen statt ersetzen - für das Protokoll. */
...
/** Ein Fehler, der sich anfühlt wie ein fehlendes Dateisystem-Objekt. */
...
const browserBackend: StorageBackend = {
	async exists(path) {
		...
		// Es gibt keine Ordner, der Pfad ist Teil des Schlüssels - "logs" allein
		// steht also nie da. Wer nach einem Ordner fragt, meint trotzdem "liegt da
		// etwas drin?", und genau das beantwortet die Präfixsuche. Ohne sie sieht
		// jeder Aufrufer, der erst auf den Ordner prüft, einen leeren Bestand.
		const prefix = path.endsWith("/") ? path : `${path}/`;
		const keys = await tx<IDBValidKey[]>("readonly", (s) => s.getAllKeys());
		return keys.some((k) => String(k).startsWith(prefix));
	},

	// Es gibt keine Ordner - der Pfad ist Teil des Schlüssels. Nichts zu tun.
	async mkdir() {},
	...
	async stat(path) {
		const v = await tx<string | undefined>("readonly", (s) => s.get(path));
		if (v === undefined) throw new NotFound(path);
		// Länge in Bytes, nicht in Zeichen: store.ts entscheidet daran, ob eine
...
/** Im Browser: den Bestand in IndexedDB führen. Einmal beim Start rufen. */
export function useBrowserStorage(): void {
	backend = browserBackend;
}

/** Ob gerade die Browser-Ablage läuft - nicht dasselbe wie `isTauri()`, siehe Aufrufer. */
