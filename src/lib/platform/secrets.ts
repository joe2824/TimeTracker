...
// ein selbstgebauter Schutz mit dem Schlüssel daneben täuscht Sicherheit vor.
import { isTauri } from "./env";

export interface ProtectedSecret {
	/** Der abzulegende Wert (base64). */
	data: string;
	/** Ob das Betriebssystem ihn wirklich geschützt hat. */
	protected: boolean;
}
...
export async function unprotectSecret(data: string, wasProtected: boolean): Promise<string> {
	if (!isTauri()) {
		if (wasProtected) {
			// Eine Ablage, die auf dem Rechner geschützt wurde, lässt sich im
			// Browser nicht öffnen. Das ist kein Fehler, sondern der Sinn der Sache.
