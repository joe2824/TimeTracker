// Der abgelegte Schlüssel darf nie wieder Bytes herausgeben - aber alles
// können, was der laufende Betrieb braucht: ver-/entschlüsseln UND die
...
describe("keyStore", () => {
	...
	it("rechnet dieselben Kennungen wie das exportierbare Original", async () => {
		// Der Punkt, an dem der Abgleich hängt: bucketFor und vaultProof laufen
		// beim Wiedereinstieg auf dieser Kopie. Kämen dort andere Werte heraus -
		// oder würde geworfen -, fände das Gerät seine eigenen Monate nicht mehr.
...
describe("Ablage von vor der Umbenennung", () => {
	...
	it("erkennt sie und raeumt sie weg", async () => {
		// Übernehmen lässt sie sich nicht: dort lag ein einzelner, nicht-
		// exportierbarer AES-Schlüssel, und der HMAC-Teil bräuchte dessen Bytes.
		// Bleibt sie liegen, liegt sie für immer im Profil.
