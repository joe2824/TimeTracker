// Der hinterlegte Schlüssel nach einer abgelaufenen Sitzung.
//
// Abgelaufen heisst nicht abgemeldet: der Schlüssel bleibt liegen, damit die
// nächste Passkey-Anmeldung die Daten wieder öffnet, ohne dass jemand 24
// Wörter abtippt. Er darf aber NUR an dasselbe Konto zurück.
...
/** Ein Gerät, das schon einmal an einem Konto hing. */
...
/** Der Abgleich, den #persistLink anstösst, geht hier ins Leere. */
...
describe("unlockWithStoredKey", () => {
	it("nimmt den hinterlegten Schluessel, wenn dasselbe Konto sich anmeldet", async () => {
		...
		// Derselbe Schlüssel, nicht irgendeiner: sonst gingen die Daten nicht auf.
	...
	it("erbt die Kennung NICHT, wenn ein anderer Schluessel dazukommt", async () => {
		// Der Weg, der das ausgelöst hat: Profil hing an Konto A, wird dann zu
		// Konto B gekoppelt. Die Kopplung kannte die Kennung früher nicht - blieb
		// sie stehen, gab unlockWithStoredKey B's Schlüssel an A.
		await linkedDevice("konto-a");
		expect(await account.unlockWithStoredKey(URL, "konto-a")).toBe(true);
	...
	it("sagt nein, wenn der Schluessel ohne Kontokennung liegt", async () => {
		// Ältere device.json kennen das Feld nicht. Ohne Kennung ist nicht zu
		// entscheiden, wem der Schlüssel gehört - dann lieber die Phrase.
		const key = await createVaultKey();
