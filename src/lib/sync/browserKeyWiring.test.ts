...
import { LOG_DIR, flushLog, logInfo } from "../log";
...
describe("Abmelden", () => {
	// Nach dem Abmelden darf im Browser nichts Lesbares zurückbleiben - weder die
	// verschlüsselten Dateien noch der Schlüssel, der sie öffnet. Der nächste
	// Mensch an diesem Rechner sähe sonst die Zeiten des vorigen.
	...
	it("laesst keine Datei mit Inhalt zurueck", async () => {
		...
		// device.json bleibt mit Absicht: darin steht die Gerätekennung, damit ein
		// erneutes Koppeln dasselbe Gerät wiedererkennt. Sonst nichts.
		const left = (await storage.readDir("data")).map((e) => e.name);
		expect(left.filter((n) => n !== "device.json")).toEqual([]);
	});

	it("nimmt die Protokolle mit", async () => {
		await linkedWithData();
		// Darin stehen Serveradresse, Geraetekennungen und wann welches Konto
		// verknuepft war - auf einem geteilten Rechner liest das der Naechste.
		logInfo("Konto verknüpft", { server: "http://localhost:5173" });
		await flushLog();
		expect((await storage.readDir(LOG_DIR)).length).toBeGreaterThan(0);

		await account.logout();

		expect(await storage.readDir(LOG_DIR).catch(() => [])).toEqual([]);
	});
