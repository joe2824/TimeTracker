// Der Passkey muss die Daten allein öffnen können - schon beim ersten Mal in
// einem neuen Browser. Die 24 Wörter sind der Weg zurück, nicht der Weg hinein.
...
const server = vi.hoisted(() => ({
	...
	/** Läuft bei jedem Abruf der Verpackungen - für Aussagen über die Reihenfolge. */
	onWraps: (() => {}) as () => void,
	/** Wie oft eine Anmelde-Aufgabe geholt wurde - daran hängt der Puffer-Test. */
	loginStarts: 0
}));
...
vi.mock("./api", () => {
	...
	class Api {
		 ...
		 * EINER Transaktion. Nur so fällt hier auf, wenn eine davon fehlt.
		 */
		registerFinish(body: {
			response: { id: string };
...
/** Was der Authentifikator zurückgibt. Ohne `prf`: kein Wert bei dieser Zeremonie. */
...
describe("register", () => {
	it("legt die Passkey-Verpackung an, auch wenn das Anlegen keinen PRF-Wert hergibt", async () => {
		...
		// Die meisten Browser rücken den Wert erst bei einer Anmeldung heraus.
...
describe("login", () => {
	...
	it("holt die vorgeladene Aufgabe nur einmal und wirft sie danach weg", async () => {
		...
		// Der Server hat die Aufgabe beim Nachsehen gelöscht: die nächste
	...
	it("neues Gerät, nur der Passkey - mehr braucht es nicht", async () => {
		...
		// nichts, der Passkey ist über die Cloud mitgekommen.
...
describe("reunlockWithPasskey", () => {
	...
	it("fragt gar nicht erst, wenn kein Passkey die Daten öffnen kann", async () => {
		...
		// wurde bisher nach genau dem Passkey gefragt, der nichts öffnen kann.
		await expect(
