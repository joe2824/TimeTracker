...
//
// Es gehört vor den vorgezogenen Teil: da wartet jemand. Sobald nur noch die
// Historie im Hintergrund läuft, muss es weg - sonst sperrt es die App zu,
// während das Hinweisband daneben sagt, man könne schon arbeiten.
...
/** Einträge in einem Monat, verteilt auf verschiedene Tage. */
...
/**
 * Das Gerät auf Anfang stellen: lokal leer, Stand 0, vorgezogene Monate gesetzt.
 * Der Server behält alles - genau die Lage nach einer frischen Verknüpfung.
 */
async function asIfFreshlyLinked(): Promise<void> {
	...
	// Was das Einrichten angezeigt hat, gehört nicht in die Messung.
...
describe("Lade-Modal beim gestuften Abruf", () => {
	it("endet mit dem vorgezogenen Teil, waehrend die Historie noch laeuft", async () => {
		try {
			...
			// Die Historie hängt am Gatter - der vorgezogene Teil ist durch.
			expect((await store.loadEntries(monthKey(Date.now()))).length).toBe(25);
			expect(await store.loadEntries(OLD)).toEqual([]);
			// Also: Modal abgeschlossen, nicht mehr "wird geladen". Wäre es nie
			// aufgegangen, stünde hier null - der vorgezogene Teil brachte 25
			// Einträge, und die gehören gemeldet.
	...
	it("bleibt weg, wenn nur die Historie laeuft", async () => {
		// Der Backfill zieht 25 alte Einträge. Ohne die Unterscheidung stünde das
		...
		try {
			...
			// Der vorgezogene Teil brachte nur Aktivitäten und Einstellungen -
			// nichts, wofür sich ein Modal lohnt.
			expect(account.bulkSync).toBeNull();

			server.release();
			await waitFor(async () => (await store.loadEntries(OLD)).length === 25);
			// Die 25 alten Einträge sind da - und zwar ohne dass die App dabei
