// Die dauerhafte Kennung dieses Geräts.
import { loadDevice, updateDevice } from "../store";
import { logInfo } from "../log";

let cached: string | null = null;

/** Die Kennung dieses Geräts, angelegt beim ersten Aufruf. */
export async function deviceId(): Promise<string> {
	...
	await updateDevice((info) => {
		...
		// angelegt haben - dann gilt deren, sonst hätten zwei Aufrufe zwei.
		if (info?.id) {
			id = info.id;
			created = false;
			return null;
