...
const activity = (id: string, over: Partial<Activity> = {}): Activity =>
	anActivity(id, { name: "Entwicklung", sortOrder: 1, favorite: false, ...over });
...
describe("Vom Konto loesen", () => {
	it("laesst jeden Eintrag inhaltlich unveraendert", async () => {
		...
		// Feld für Feld derselbe Eintrag - nur ohne Stempel.
	...
	it("faesst einen Monat ohne Stempel gar nicht erst an", async () => {
		...
		// verknüpft ist, stempelt jeder Schreibvorgang. Genau so wird es hier
		// nachgestellt - erst der alte Bestand, dann die Verknüpfung.
		stopTracking();
		await store.saveEntries("2026-05", [entry("alt")]);
		await startTracking("dieses-geraet");
		await store.saveEntries(MONTH, [entry("e1", { rev: 1 })]);
	...
	it("raeumt die Outbox", async () => {
		...
		// Was offen war, bezog sich auf ein Konto, das dieses Gerät nicht mehr
		// hat. Stehen zu lassen hiesse: beim nächsten Koppeln wird als Erstes
		// eine Handvoll uralter Änderungen hochgeladen.
	...
	it("erzeugt selbst keine neuen Vormerkungen", async () => {
		await store.saveEntries(MONTH, [entry("e1", { rev: 1 })]);
		await detachLocalData();
		// Das Abstreifen ist ein Schreibvorgang. Würde der Haken ihn als Änderung
		// dieses Geräts nehmen, wäre die Outbox danach wieder voll - mit genau
		// dem, was gerade weggeräumt wurde.
		expect(pendingChanges()).toHaveLength(0);
	});
