...
describe("pickWinner", () => {
	...
	it("entscheidet bei Gleichstand auf BEIDEN Geraeten gleich", () => {
		// Der Punkt der Gerätekennung als Stichentscheid. Würde jedes Gerät bei
		// Gleichstand "meins" wählen, überschrieben sie sich endlos gegenseitig.
		const a = s({ deviceId: "aaa" });
		const b = s({ deviceId: "bbb" });
		// Aus Sicht von Gerät A ist a lokal, aus Sicht von B ist b lokal.
		expect(pickWinner(a, b)).toBe("remote"); // A wählt b
		expect(pickWinner(b, a)).toBe("local"); // B wählt ebenfalls b
	});
...
describe("mergeRecord", () => {
	...
	it("legt fuer einen Loeschmarker ohne lokalen Stand nichts an", () => {
		// Sonst entstünde aus einer Löschung, die uns nie betraf, eine leere Zeile.
		const r = mergeRecord(
	...
	it("nimmt den Serverstand auch dann, wenn er AELTER ist - solange nichts offen ist", () => {
		// Ohne offene eigene Änderung ist der Server die Wahrheit. Ein älterer
		// Stempel heisst hier nicht "veraltet", sondern dass das eigene Gerät
	...
	it("meldet es, wenn eine eigene Aenderung unterliegt", () => {
		// Der einzige Fall, den ein Mensch erfahren muss: seine Änderung ist weg,
	...
	it("laesst eine juengere Bearbeitung eine aeltere Loeschung ueberleben", () => {
		// "Löschung gewinnt immer" klänge sicherer, machte aber das Wiederanlegen
		// eines versehentlich gelöschten Eintrags unmöglich.
		const r = mergeRecord(
	...
	it("nimmt einen Loeschmarker an, auch wenn die Zeit gleich ist", () => {
		...
		// Einträge eines Jahres mit demselben Date.now(). Fällt das mit dem
		// updatedAt eines gerade erst angelegten Eintrags zusammen, standen hier
		// zwei gleiche Stempel - und die Abkürzung für "derselbe Stand" nahm
		// den Löschmarker nicht zur Kenntnis. Das andere Gerät behielt den
		// Eintrag und schob ihn beim nächsten Abgleich wieder hoch.
		const r = mergeRecord(
			{
	...
	it("kommt auf beiden Geraeten zum selben Ergebnis", () => {
		// Die Zusage, an der alles hängt: derselbe Code, dieselbe Ausgangslage,
...
describe("resolveOpenEntries", () => {
	...
	it("entscheidet nach der juengsten HANDLUNG, nicht nach der Startzeit", () => {
		// Wer um 10 am Rechner startet und dabei auf 8 Uhr zurückdatiert, meint
	...
	it("laesst eine geratene Mitternachts-Fortsetzung den laufenden Timer nicht schliessen", () => {
		...
		const yesterday = e("yesterday", { startTs: ts(15, 9), endTs: ts(15, 18), rev: 2 });
		const continuation = e("continuation", { startTs: midnight, updatedAt: 9_000_000 });
		const real = e("real", { startTs: ts(16, 8), updatedAt: 1000, rev: 1, deviceId: "handy" });

		const fix = resolveOpenEntries([yesterday, continuation, real]);

		expect(fix.map((x) => x.id)).toEqual(["continuation"]);
	...
	it("laesst eine lueckenlos anschliessende Fortsetzung normal mitspielen", () => {
		...
		const midnight = startOfNextDay(ts(15, 9));
		const yesterday = e("yesterday", { startTs: ts(15, 9), endTs: midnight, rev: 2 });
		const continuation = e("continuation", { startTs: midnight, updatedAt: 9_000_000, rev: 3 });
		const real = e("real", { startTs: ts(16, 8), updatedAt: 1000, rev: 1, deviceId: "handy" });

		const fix = resolveOpenEntries([yesterday, continuation, real]);

		expect(fix.map((x) => x.id)).toEqual(["real"]);
	});
