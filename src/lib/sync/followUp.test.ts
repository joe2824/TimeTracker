...
async function link(g: FakeDevice, opts: { ownDataOnly?: boolean } = {}): Promise<void> {
	await on(g, async (engine) => {
		await engine.sync();
		// Was AccountState prüft, bevor es nachliest: gehört der ungestempelte
		// Bestand überhaupt zu DIESEM Konto?
...
/**
 * Dasselbe Gerät an einem ANDEREN Konto: neuer Server, neuer Schlüssel - und
 * der Vermerk, den `#persistLink` beim Wechsel hinterlässt (der Bestand
 * gehört noch dem alten Konto).
 */
...
describe("Nachlese: was der Schreib-Haken nie gesehen hat", () => {
	it("laedt den GESAMTEN lokalen Bestand hoch, nicht nur das danach Geaenderte", async () => {
		...
		// Jetzt verknüpft er ein Konto und gleicht ab.
		await link(desktop);
	...
	it("ueberschreibt die Einstellungen des Kontos NICHT mit den Voreinstellungen des Neulings", async () => {
		...
		// Ein zweites Gerät, das schon einmal lief und dabei Voreinstellungen
		// weggeschrieben hat - der häufigste Fall im Browser.
		const browser = new FakeDevice("browser");
		...
		// Und der Rechner bekommt seinen eigenen Wert nicht zerschossen zurück.
		await link(desktop);
	...
	it("traegt den Bestand NICHT in ein fremdes Konto", async () => {
		...
		// Jetzt hängt dasselbe Gerät an einem ANDEREN Konto - anderer Schlüssel.
	...
	it("nimmt die Merkliste des vorigen Kontos nicht mit", async () => {
		// Der Stempel-Wächter allein reicht nicht: `#pushAll` liest die OUTBOX -
