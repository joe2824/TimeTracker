...
describe("diffAndStamp", () => {
	...
	it("stempelt einen Bestandsdatensatz ohne Spuren nach", () => {
		// Daten aus einer Fassung ohne Serveranbindung: inhaltlich unverändert,
		// aber nie gestempelt. Ohne diesen Fall bliebe der Altbestand für immer
		// unsynchronisiert – und genau das ist der häufigste Fall beim Umstieg.
		const old = e("1");
		const { changes, stamped } = diffAndStamp([old], [{ ...old }], DEV, NOW);
		expect(changes.changed).toHaveLength(1);
		expect(stamped[0].updatedAt).toBe(NOW);
	...
	it("meldet eine Loeschung samt der Fassung des Datensatzes", () => {
		// Die Fassung muss mit: der Server nimmt eine Löschung nur auf seinem
		// aktuellen Stand an, und nach dem lokalen Löschen ist sie sonst nirgends
		// mehr zu holen.
	...
	it("stempelt beim zweiten Speichern nicht erneut", () => {
		// Der Stempel selbst darf nicht als Änderung zählen, sonst stempelt jedes
		// Speichern erneut und der Abgleich lädt denselben Stand endlos hoch.
	...
	it("reicht den Serverstand des Vorgaengers weiter", () => {
		// Damit der Abgleich weiss, auf welchem Serverstand die Änderung aufsetzt –
