...
/** Was ein Authentifikator mit PRF liefern würde: 32 stabile Bytes. */
...
describe("Datensaetze versiegeln", () => {
	...
	it("nimmt bei jedem Mal einen neuen Zufallswert", async () => {
		// Zweimal dasselbe zu verschlüsseln darf nicht zweimal dasselbe ergeben,
		// sonst sieht der Server, dass sich nichts geändert hat. JWE-Compact-Form:
	...
	it("laesst sich nicht auf einen anderen Datensatz umhaengen", async () => {
		// Der Server kennt den Inhalt nicht, könnte aber Chiffrate vertauschen.
...
describe("Zeitraum-Kennung", () => {
	...
	it("verraet den Monat nicht", async () => {
		...
		// Auf "enthält nicht '07'" zu prüfen wäre Unfug: in 32 zufälligen
		...
		// Bei einer guten Streuung stimmt rund ein Sechzehntel der Stellen zufällig
		// überein. Die Hälfte wäre ein Hinweis darauf, dass der Monat durchschlägt.
		expect(samePlaces).toBeLessThan(16);
	});

	it("ist bei zwei Konten verschieden, auch fuer denselben Monat", async () => {
		// Sonst könnte der Server zwei Konten daran erkennen, dass sie dieselben
		// Zeiträume haben.
...
describe("Wiederherstellungs-Phrase", () => {
	...
	it("erkennt ein vertipptes Wort an der Pruefsumme", () => {
		...
		// EIN Wort zu ersetzen und "fällt auf" zu erwarten wäre ein Test, der
		// gelegentlich grundlos rot ist: bei 24 Wörtern hat BIP39 nur 8
		// Prüfbits, ein falsches Wort passt also mit 1:256 zufällig doch.
	...
	it("nimmt bei zwei Verpackungen verschiedenen Zufall", async () => {
		...
		expect(a).not.toBe(b);
		// Beide öffnen trotzdem denselben Vault.
...
describe("Kopplung eines neuen Geraets", () => {
	it("das neue Geraet oeffnet, was das entsperrte fuer es verpackt hat", async () => {
		...
		// Auf dem ENTSPERRTEN Gerät:
		const packet = await wrapForDevice(vault, publicSide);
		// Zurück auf dem neuen Gerät:
	...
	it("weist ein Paket ab, dessen fluechtiger Schluessel im Header fehlt", async () => {
		// `epk` (der flüchtige öffentliche Schlüssel) liegt im geschützten
		// JWE-Header - der ist Teil der authentisierten Daten. Ein Paket ohne ihn
		// (oder mit verändertem Header) fällt beim Öffnen durch, weil die
		// Prüfsumme nicht mehr passt - unabhängig davon, was genau am Header fehlt.
		const vault = await createVaultKey();
		const freshPair = await createPairingKeyPair();
...
describe("mehrere Wege zum selben Vault", () => {
	it("Phrase, Passkey und Geraet oeffnen denselben Schluessel", async () => {
		// Das ist der Kern des Entwurfs: EIN Vault-Schlüssel, mehrere Öffner.
...
describe("Der Kopplungscode", () => {
	...
	it("rechnet, was eine unabhaengige Umsetzung auch rechnet", async () => {
		...
		// dieses Programms: SHA-256 über die Bytes 0..64, davon die obersten 60 Bit
		// zu zwölf Stellen a fünf Bit.
	...
	it("ist fuer zwei Schluessel ein anderer", async () => {
		// Die Eigenschaft, auf der alles beruht: ein untergeschobener Schlüssel
...
describe("Das Abhol-Geheimnis", () => {
	...
	// sein - ein Mensch soll ihn vergleichen. Zum Abholen des Geräte-Tokens
	// taugt er deshalb nicht: dafür gibt es dieses Geheimnis, das das Gerät
	// behält und von dem der Server nur den Hash sieht.
	...
	it("wuerfelt jedes Mal neu", async () => {
		// Zwei Geräte, die gleichzeitig koppeln, dürfen sich nicht dasselbe
...
describe("checkedPairingKey", () => {
	...
	it("weist einen untergeschobenen Schluessel ab", async () => {
		// GENAU der Angriff, gegen den die Bindung da ist: das neue Gerät
		// hinterlegt seinen Schlüssel und zeigt dessen Code an. Wer den Server
		// beherrscht, tauscht den hinterlegten Schlüssel gegen einen eigenen -
		// und bekäme vom bestätigenden Gerät den Vault-Schlüssel dagegen
		// verpackt. Danach könnte er jeden Datensatz des Kontos lesen.
	...
	it("weist auch einen verbogenen Schluessel ab", async () => {
		// Nicht nur ein ganz anderer Schlüssel: ein einziges gekipptes Bit reicht.
		const real = await exportPairingPublicKey(await createPairingKeyPair());
	...
	it("der geprüfte Schluessel oeffnet das Paket wirklich", async () => {
		// Ende zu Ende: was checkedPairingKey durchlässt, ist der Schlüssel, mit
		// dem das neue Gerät sein Paket auch aufbekommt.
...
describe("Wiederherstellung ueber die Phrase", () => {
	...
	it("folgt der Entropie der Phrase, nicht ihrem Text", async () => {
		// Der Schlüssel entsteht aus der ENTROPIE der Phrase (siehe kekFromPhrase),
		// nicht aus ihrem Text - sonst hätten Schlüssel und Kennung verschiedene
		// Vorstellungen davon, wann zwei Phrasen gleich sind, und eine gültige
		// Schreibweise fände kein Konto.
		const p = createRecoveryPhrase();
	...
	it("die Kennung oeffnet die Verpackung NICHT", async () => {
		// Der Punkt, an dem alles hängt: Kennung und Schlüssel entstehen aus
		// derselben Phrase, aber über getrennte Wege. Wären es dieselben Bytes,
		// gäbe der Server mit der Kennung den Schlüssel heraus.
		const key = await createVaultKey();
		const p = createRecoveryPhrase();
	...
	it("der Nachweis gibt den Schluessel nicht her", async () => {
		...
		// Der Schlüssel selbst darf im Nachweis nicht auftauchen.
	...
	it("der ganze Weg: Phrase rein, Vault-Schluessel raus", async () => {
		...
		// Was auf dem neuen Gerät passiert: Kennung rechnen, Verpackung holen,
		// öffnen, Nachweis rechnen.
		expect(await recoveryLookupId(p)).toBe(id);
		const back = await unwrapWithPhrase(wrap, p);
		expect(await vaultProof(back)).toBe(proof);

		// Und der Schlüssel ist wirklich derselbe.
