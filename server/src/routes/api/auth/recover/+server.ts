// Wiederherstellung mit der Phrase: Konto zurückholen, wenn weder ein zweites
// Gerät noch ein Passkey da ist.
...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	// Die jüngste, falls je mehr als eine dastehen sollte.
	const wrap = user
		? locals.db
				.select()
				.from(keyWraps)
				...
				.get()
		: undefined;
	// Absichtlich dieselbe Meldung für "gibt es nicht" und "hat keine
	// Verpackung": sonst lässt sich herausfinden, welche Konten es gibt.
	if (!user || !wrap) error(404, "Zu dieser Phrase gibt es kein Konto");

	const proof = body?.proof ? String(body.proof) : null;

	// Schritt 1: nur die Verpackung. Sie öffnet sich nur mit den Wörtern.
