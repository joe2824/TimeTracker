// Kopplung, Schritt 1 - auf dem NEUEN Gerät.
 ...
 * Schlüssels. Nachgerechnet wird auf den GERAETEN, nie hier.
 *
 * Dazu kommt der Hash eines Abhol-Geheimnisses. Das Geheimnis selbst bleibt auf
 * dem Gerät: der Code muss sichtbar sein, damit ein Mensch ihn vergleichen kann,
...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	if (!isClaimHash(claimHash)) {
		// Ältere Fassungen der Anwendung schicken ihn nicht mit. Ohne ihn liesse
		// sich das Geräte-Token allein mit dem Code abholen - genau das soll nicht
		// mehr gehen, also wird der Vorgang abgewiesen statt still geschwächt.
	...
	// weggeräumt, damit ein schon hinterlegtes Paket nicht verlorengeht.
	if (present && present.expiresAt >= Date.now()) {
		if (present.publicKey !== publicKey) {
			// Ein anderer Schlüssel unter demselben Code. Bei zwölf Stellen ist das
		...
		// Dasselbe gilt für das Abhol-Geheimnis: wer den Code UND den öffentlichen
		// Schlüssel kennt (beides ist sichtbar bzw. abfragbar), dürfte sonst ein
		// eigenes Geheimnis nachschieben und dem echten Gerät das Token wegnehmen.
	...
	// Der Vorgang hängt noch an keinem Konto: welches es wird, entscheidet sich
	// erst, wenn ein entsperrtes Gerät ihn bestätigt.
	locals.db
		.insert(pairings)
