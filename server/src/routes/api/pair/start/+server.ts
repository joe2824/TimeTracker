...
import { istPairingCode, istClaimHash, normalisiereCode } from "$lib/server/pairing";
...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	if (vorhanden && vorhanden.expiresAt >= Date.now()) {
		...
		if (vorhanden.claimHash && !safeEqual(vorhanden.claimHash, claimHash)) {
			error(409, "Unter diesem Code läuft bereits eine andere Kopplung");
		}
		locals.db
			.update(pairings)
