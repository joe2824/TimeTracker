...
describe("recordTelemetryPing", () => {
	...
	// Gekürzt statt abgewiesen fielen zwei Geräte mit gleichem Anfang über den
	// Eindeutigkeits-Index zu einer Zeile zusammen - eines wäre aus der Zählung
	// verschwunden, und beide bekämen ein "ok" zurück.
...
describe("getTelemetryStats", () => {
	...
	// WAU und MAU haben feste Fenster (7 und 30 Tage). `days` steuert nur, wie weit
	// der Verlauf zurückreicht, und darf die beiden Zahlen nicht beschneiden.
	it("rechnet WAU und MAU aus ihrem eigenen Fenster, nicht aus `days`", () => {
		...
		// Je Gerät ein Tag: heute, vor 3, vor 10 und vor 40 Tagen.
		...
		// Verlauf über nur 2 Tage - WAU und MAU bleiben davon unberührt.
	...
	// Heute und Gestern kommen aus eigener Abfrage, nicht aus dem Verlauf: der reicht
	// bei `days=1` nur bis heute und wüsste von gestern nichts.
