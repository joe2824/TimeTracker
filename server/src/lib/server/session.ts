...
// In Produktion erzwingt das __Host--Präfix auf Browserebene:
// Secure, HttpOnly, kein Domain-Attribut, kein Subdomain-Cookie-Theft.
// In der Entwicklung (localhost/HTTP) ist das Präfix unzulässig - daher konditionell.
...
export function setSessionCookie(cookies: Cookies, secret: string): void {
	cookies.set(SESSION_COOKIE, secret, {
		...
		// sind zusätzlich über die Herkunftsprüfung abgesichert.
		sameSite: "lax",
		// Über eine unverschlüsselte Verbindung gibt es kein Secure-Cookie - sonst
		// wäre die Anwendung auf localhost nicht zu entwickeln.
