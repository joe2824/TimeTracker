...
/** Eine Kopplung, wie sie /api/pair/start hinterlässt - Zeitpunkt frei wählbar. */
...
describe("cleanupExpired: Kopplungen", () => {
	...
	it("widerruft das Geraet einer bestaetigten, nie abgeholten Kopplung", () => {
		// Der gefährliche Fall: jemand hat bestätigt, das neue Gerät ist nie
		// erschienen. Das Token gilt, aber niemand hatte es je in der Hand - es
		// läge sonst im Klartext in der Zeile und zählte weiter als Gerät.
	...
	it("laesst ein abgeholtes Geraet in Ruhe", () => {
		// Wer abholt, löscht seine Kopplungszeile dabei selbst. Es darf nicht
		// passieren, dass das Aufräumen ein Gerät trifft, das längst läuft.
		const deviceRow = createDevice(db, ANNA, "Rechner");
		cleanupExpired(db);
...
describe("userFromSession: die Frist laeuft ab der letzten Nutzung", () => {
	...
	it("bleibt damit unbegrenzt gueltig, solange jemand die Anwendung benutzt", () => {
		...
		// Entsperren-Bildschirm. Wer täglich arbeitet, soll ihn nie sehen.
