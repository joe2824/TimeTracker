...
/**
 * Was diese Datei an der Umgebung dreht. Vitest verwendet Worker-Prozesse für
 * mehrere Test-Dateien wieder - `process.env` wird dabei NICHT zurückgesetzt,
 * stehen gelassene Werte kippen also die nächste Datei.
 */
const TOUCHED_ENV = [
	"DB_FILE",
	"DATA_DIR",
...
/** Eine Anfrage im Namen eines Geräts. */
...
/** Anfrage aus einer eigenen Adresse - so verbraucht kein Test den Vorrat des nächsten. */
...
// Das Abhol-Geheimnis. Es entsteht auf dem neuen Gerät und bleibt dort; der
// Server sieht nur den Hash. Der Code darf sichtbar sein - das Geheimnis nicht,
// und genau das trennt "abholen dürfen" von "den Code kennen".
...
/**
 * Ein Push mit Sitzungs-Cookie: der Ausweis fährt automatisch mit, deshalb
...
beforeAll(async () => {
	...
	// Gefundenes über die laufende Datenbank zurück: ein Rest aus einem früheren
	...
	// Kurz halten: sonst hängt jeder Wartetest 25 Sekunden.
	process.env.SYNC_WAIT_MS = "800";
	// Ohne den Schlüssel gäbe es den Telemetrie-Endpunkt gar nicht.
	process.env.TELEMETRY_KEY = TELEMETRY_KEY;

	db = openDb(dbFile).db;

	// Den gebauten Server starten. Er liest DB_FILE aus der Umgebung und öffnet
	// dieselbe Datei - beide Seiten sehen damit denselben Bestand.
	// Der GEBAUTE Server, nicht die Quellen: nur so ist geprüft, was später
	// wirklich läuft - samt Adapter, Kompilat und Auflösung der Abhängigkeiten.
...
afterAll(async () => {
	// Abwarten, nicht nur anstossen: ein noch offener Zuhörer hält den Port und
	// die nächste Datei im selben Prozess fände ihn belegt.
	await closeServer();
...
describe("Zugang", () => {
	...
	it("laesst kein fremdes Geraet widerrufen", async () => {
		...
		// Annas Gerät funktioniert weiterhin.
...
describe("Verpackungen", () => {
	it("ersetzt die Phrasen-Verpackung, statt eine zweite anzulegen", async () => {
		// Sonst gäbe es zwei gültige Wege zum selben Schlüssel - eine stille
		// Hintertür für eine Phrase, die jemand für ersetzt hält.
...
describe("Kopplung", () => {
	// Der Code ist auf dem GERAET der Abdruck des öffentlichen Schlüssels
	...
	// selbst der Angreifer, und seine eigene Prüfung bewiese niemandem etwas.
	// Was er prüft, ist die Form. Nachgerechnet wird drüben, in vault.test.ts.
	const CODE = "ABCDEFGHJKLM";
	const CODE2 = "NPQRSTUVWXYZ";

	it("laeuft vom Code bis zum Token durch", async () => {
		// Schritt 1 auf dem neuen Gerät - ohne Anmeldung.
		const start = await (
		...
		// Schritt 2 auf dem entsperrten Gerät.
		...
		// Schritt 3 zurück auf dem neuen Gerät.
		const claim = await (
			await api(null, "/api/pair/claim", {
				method: "POST",
				body: claimBody(start.code)
		...
		// Das frische Token gehört zu Annas Konto.
	...
	//
	// Der Kopplungscode ist der Abdruck des Geräteschlüssels. Er MUSS sichtbar
	...
	// bekäme sonst das Geräte-Token. Den Vault öffnet er damit zwar nicht,
	// aber er hätte Zugang zu den versiegelten Datensätzen - und der echte
	// Vorgang wäre abgeräumt, weil das Abholen die Zeile löscht.

	// apiVon statt api: jeder dieser Tests beginnt eigene Kopplungen, und der
	// Bremseimer für /api/pair/start hängt an der Adresse. Über api() wäre der
	// Vorrat der übrigen Tests aufgebraucht.
	it("gibt ohne Abhol-Geheimnis nichts heraus", async () => {
		...
		// Nur der Code - wie ihn jemand vom Bildschirm ablesen könnte.
	...
	it("weist ein falsches Abhol-Geheimnis wie einen unbekannten Code ab", async () => {
		...
		// gibt. So sieht Raten aus wie Raten - und die Bremse zählt es als
	...
	it("laesst kein fremdes Abhol-Geheimnis nachschieben", async () => {
		// Der öffentliche Schlüssel ist über /api/pair/approve abfragbar. Wer ihn
		// samt Code hat, dürfte sonst mit einem eigenen Geheimnis noch einmal
		// starten und dem echten Gerät das Token wegnehmen.
	...
	it("rechnet denselben Hash wie das Geraet", () => {
		...
		// Rechnen Server und Gerät verschieden, koppelt gar nichts mehr - und
	...
	it("beginnt keine Kopplung ohne Abhol-Geheimnis", async () => {
		// Ältere Fassungen der Anwendung schicken keinen mit. Der Vorgang wird
		// abgewiesen, statt still auf die schwächere Regel zurückzufallen.
	...
	it("laesst einen laufenden Vorgang nicht von aussen ueberschreiben", async () => {
		...
		// dazwischenfahren - sonst läge unter einem Code, den jemand gerade
		// abliest, plötzlich ein anderer Schlüssel.
		...
		// Und der echte Schlüssel steht noch da.
	...
	it("nimmt denselben Vorgang noch einmal an, ohne das Paket zu verlieren", async () => {
		// Derselbe Schlüssel ergibt denselben Code: ein zweiter Anlauf desselben
		// Geräts landet zwangsläufig auf demselben Vorgang.
		...
		// Das hinterlegte Paket hat den zweiten Anlauf überlebt.
		const claim = await (
...
describe("Ereigniskanal", () => {
	it("meldet den aktuellen Stand und weckt bei einer Aenderung", async () => {
		...
		// Ein zweites Gerät schreibt - der Kanal muss davon erzählen.
		await api(annaToken, "/api/sync", {
			method: "POST",
			body: JSON.stringify({ records: [rec("e1")] })
		});
	...
	it("erzaehlt nicht von den Aenderungen eines fremden Kontos", async () => {
		...
		// Kein Weckruf innerhalb einer kurzen Frist. Ohne die Frist wäre der Test
...
describe("Warteschleife statt Ereigniskanal", () => {
	it("kommt sofort zurueck, wenn der Server schon weiter ist", async () => {
		...
		// Nicht gewartet: sonst verpasst ein Client jede Änderung, die zwischen
	...
	it("haelt offen und antwortet, sobald geschrieben wird", async () => {
		...
		// Erst schreiben, wenn die Anfrage wirklich hängt - sonst fängt sie der
		// Schnellweg oben ab und der Test prüft nichts.
	...
	it("weckt nicht bei der Aenderung eines fremden Kontos", async () => {
		...
		// Sie kommt nach Ablauf der Wartezeit zurück - aber ohne Änderung.
...
describe("Konto aufloesen", () => {
	/** Was Anna alles beim Server hat, bevor sie es auflöst. */
	...
	it("laesst eine blosse Sitzung nicht genuegen", async () => {
		// Der Kern der Sache: ein Cookie fährt bei jeder Anfrage automatisch mit.
		// Es beweist, dass irgendwann jemand angemeldet war - nicht, dass gerade
		// jetzt jemand zustimmt. Ohne frische Bestätigung passiert nichts.
	...
	it("weist eine erfundene Bestaetigung ab", async () => {
		...
		// Datenbank) - dann gibt es auch nichts zu bestätigen, und der Weg über
		// die Sitzung ist verschlossen. Genau richtig: einen Passkey vorzutäuschen
		// darf nicht gehen.
	...
	it("nimmt das Geraete-Token als Nachweis - es ist der Zugangscode selbst", async () => {
		// 256 Bit Zufall, genau einmal bei der Kopplung übertragen, in keinem
		// Browser-Kontext, fährt nirgends automatisch mit. Wer es hat, hat das
		// gekoppelte Gerät - und damit ohnehin den Vault-Schlüssel.
	...
	it("laesst das Konto daneben unangetastet", async () => {
		...
		// die Löschung wirklich auf ein Konto eingeschränkt war.
	...
	it("der Zugang gilt danach nicht mehr", async () => {
		...
		// Das Token ist nicht nur widerrufen - das Gerät dahinter gibt es nicht
	...
	it("meldet einen Konflikt gegen Fassung 0 fuer einen Datensatz, den es nicht gibt", async () => {
		// Die Lage nach einem Auflösen: das Gerät hält noch Fassungsnummern von
...
describe("Geraet loesen", () => {
	it("loest ohne Angabe das aufrufende Geraet selbst", async () => {
		// Der Weg beim Entkoppeln: das Gerät kennt seine eigene Kennung beim
	...
	it("laesst das Konto und seine Daten dabei stehen", async () => {
		...
		// Das andere Gerät arbeitet weiter, und die Daten sind vollständig da.
		// Genau hierin unterscheidet sich "Gerät lösen" vom Auflösen.
	...
	it("gibt eine Sitzung ohne Geraet nicht als Geraet aus", async () => {
		// Ohne Token und ohne Angabe gibt es nichts zu lösen. Wäre das ein
		// stiller Erfolg, hätte der Aufrufer den Eindruck, etwas sei passiert.
...
// leerlaufen lassen. Ihr Zustand lebt im Serverprozess und lässt sich von aussen
// nicht zurücksetzen - ein leergeratener Eimer würde hier sonst als Fehler
// erscheinen, obwohl nichts kaputt ist.
describe("Warten beim Koppeln", () => {
	it("bremst das Warten nicht aus", async () => {
		// Die Oberfläche fragt im Zwei-Sekunden-Takt nach, ob jemand bestätigt
		// hat, und ein Kopplungscode gilt zehn Minuten. Das sind bis zu
		// dreihundert Anfragen für einen völlig normalen Vorgang.
...
describe("Bremse und Herkunft", () => {
	it("bremst das Abfragen eines Kopplungscodes", async () => {
		...
		// Geräte-Token heraus. Der Code hat vierzig Bit - rechnerisch nicht zu
		// raten. Aber "rechnerisch nicht" ist eine Aussage über einen Angreifer,
		// der ehrlich rechnet, nicht über einen, der oft fragt.
		let throttled = false;
		for (let i = 0; i < 40; i++) {
			const res = await api(null, "/api/pair/claim", {
				method: "POST",
	...
	it("laesst ein Geraete-Token ohne Herkunftspruefung durch", async () => {
		...
		// fährt nirgends automatisch mit.
		const res = await api(annaToken, "/api/sync", {
			method: "POST",
...
describe("Ohne Reverse-Proxy davor", () => {
	it("bremst auch dann, wenn die Adresskopfzeile fehlt", async () => {
		...
		// Was genau herauskommt, hängt an den Einladungscodes - nur ein
...
describe("Herkunft unter verschiedenen Namen", () => {
	// Derselbe Container ist über localhost, 127.0.0.1, den Rechnernamen und die
...
describe("Verwaltung", () => {
	...
	it("nennt dem Verwalter die Zahl der Konten", async () => {
		...
		// Anna und Bodo. Gezählt werden Konten - nicht Geräte und nicht Telemetrie,
	...
	it("nimmt den alten Feldnamen gueltigTage weiter an", async () => {
		makeAdmin(ANNA);
		// Eine ältere Desktop-Anwendung schickt noch gueltigTage. Ohne den Rückfall
		// bekäme sie stillschweigend einen Code ganz ohne Frist.
		const res = await api(annaToken, "/api/admin/invites", {
			method: "POST",
...
describe("Herkunft unter einem fremden Namen", () => {
	// ORIGIN sagt "localhost:<Port>". Jemand erreicht den Dienst aber über den
	...
	it("laesst durch, wenn Origin und Host zueinander passen", async () => {
		const session = createSession(db, ANNA);
		// Mit `fetch` geht das nicht: die Host-Kopfzeile ist geschützt und wird
		// immer auf die tatsächliche Zieladresse gesetzt. Genau die soll hier aber
...
describe("Passkeys verwalten", () => {
	...
	it("schliesst beim Anlegen die vorhandenen aus", async () => {
		// Sonst legt derselbe Authentifikator einen zweiten Passkey für dasselbe
		// Konto an - und der Mensch glaubt, er hätte jetzt zwei Wege, obwohl beide
		// an demselben Gerät hängen.
	...
	it("entfernt einen von mehreren - samt seiner Verpackung", async () => {
		...
		// Die Verpackung ist ohne ihren Passkey nicht mehr zu öffnen - sie stehen
		// zu lassen hiesse, eine Tür ohne Schlüssel zu verwahren.
...
describe("Konto von einem Geraet aus anlegen", () => {
	...
	it("bremst das Anlegen vom Rechner aus", async () => {
		...
		// offener Registrierung entsteht dabei jedes Mal ein Konto samt Gerät.
		let throttled = false;
		for (let i = 0; i < 40; i++) {
			// Eigene Absenderadresse: der Eimer hängt daran, und die anderen Tests
	...
	it("legt Konto und Geraet in einem Zug an", async () => {
		...
		// Das Token trägt sofort - ohne das wäre das Konto unerreichbar, denn
	...
	/** Ein frisches Konto samt Gerät - der Weg ohne Passkey. */
	...
	it("kann sofort abgleichen", async () => {
		...
		// hoch. Ein Konto, das erst noch einen Passkey braucht, wäre ein Umweg.
	...
	it("nimmt die Verpackung der Phrase an", async () => {
		...
		// Ohne sie wäre das Konto an genau dieses eine Gerät gebunden.
...
describe("Herkunft ohne Sitzung", () => {
	...
	it("weist eine fremde Seite MIT Sitzung weiterhin ab", async () => {
		// Hier fährt der Ausweis automatisch mit - und genau darum geht es.
		const res = await pushWithSession(ANNA, { origin: "https://boeswillig.example" });
		expect(res.status).toBe(403);
	});
});
...
describe("Anlegen ohne Namen", () => {
	it("kommt ohne Anzeigename aus", async () => {
		...
		// dieses Konto nie - es hat keinen Passkey. Danach zu fragen wäre eine
		...
		// Ohne Namen steht die Kennung da - hässlich und ehrlich.
...
describe("Wiederherstellung mit der Phrase", () => {
	...
	it("verraet nicht, ob es ein Konto gibt", async () => {
		// Dieselbe Meldung für "kenne ich nicht" und "hat keine Verpackung" -
		// sonst lässt sich durchprobieren, welche Konten existieren.
		const res = await apiFrom(null, "/api/auth/recover", {
			method: "POST",
	...
	it("gibt OHNE Nachweis kein Geraete-Token", async () => {
		...
		// bekommt die Verpackung - die er nicht öffnen kann - und sonst nichts.
		await store(annaToken, { recoveryId: "kennung-anna", vaultProof: "beweis-anna" });
		const res = await apiFrom(null, "/api/auth/recover", {
			method: "POST",
			body: JSON.stringify({ recoveryId: "kennung-anna" })
	...
	it("meldet mit richtigem Nachweis ein Geraet an", async () => {
		...
		// Und das Token trägt sofort - die Daten sind ja noch da, nur der Zugang
		// war weg.
		const self = await (await api(deviceToken, "/api/me")).json();
	...
	it("legt den Nachweis nur als Hash ab", async () => {
		// Sonst genügte ein Datenbankabzug: abschreiben, zurückschicken, Token.
	...
	it("nimmt Kennung und Nachweis nur zusammen an", async () => {
		// Eines allein würde das andere überschreiben.
		await store(annaToken, { recoveryId: "kennung-anna", vaultProof: "beweis" });
	...
	it("bremst das Durchprobieren des Nachweises", async () => {
		// Eine feste Adresse für alle Versuche - die Bremse zählt je Aufrufer.
		await store(annaToken, { recoveryId: "kennung-anna", vaultProof: "beweis-anna" });
		let throttled = false;
...
describe("Telemetrie", () => {
	/** Ein Ping mit eigener Adresse - sonst nimmt ein Test dem nächsten die Bremse weg. */
	...
	// Die PWA liegt im Server-Image, das für alle Betreiber dasselbe ist -
	// einen Schlüssel kann sie gar nicht mitbekommen. Sie weist sich mit ihrer
	// Anmeldung aus.
	...
	// 403 und nicht 401: ein Schlüssel, den dieser Server nicht annimmt, wird
	// auch beim nächsten Versuch keiner sein. Der Client liest 403 als "nicht
	...
	it("weist einen Ping mit falschem Schluessel dauerhaft ab", async () => {
		...
		// Gleiche Länge, anderer Inhalt: der Vergleich darf nicht am Präfix hängen.
