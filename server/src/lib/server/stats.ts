...
// Erfasst ausschliesslich: Datum, anonyme Gerätekennung, App-Version und Betriebssystem/Plattform.
import { eq, gte, sql } from "drizzle-orm";
import type { Db, DbLike } from "./db/index";
import { telemetryPings } from "./db/schema";
import type Database from "better-sqlite3";

/** Kürzeste noch brauchbare Gerätekennung. */
export const MIN_DEVICE_ID_LEN = 4;
/**
 * Längste Gerätekennung. Der Endpunkt weist längere ab, statt sie hier zu
 * kürzen: zwei Geräte mit gleichem Anfang fielen sonst über den
 * Eindeutigkeits-Index zu einer Zeile zusammen.
 */
export const MAX_DEVICE_ID_LEN = 64;

/** Plattformen, die die Anwendung meldet. Alles andere wird nicht übernommen. */
...
/** Ziffern, Punkte und die Zusätze einer Vorabversion - mehr hat eine Version nicht. */
const VERSION_RE = /^[0-9A-Za-z][0-9A-Za-z.+-]{0,31}$/;

/** Wofür kein brauchbarer Wert ankam. */
...
export interface TelemetrySummary {
	today: number;
	yesterday: number;
	/** Letzte 7 Tage (eindeutige Geräte), unabhängig vom abgefragten Verlauf. */
	wau: number;
	/** Letzte 30 Tage (eindeutige Geräte), unabhängig vom abgefragten Verlauf. */
	mau: number;
	/** Pings im abgefragten Verlauf - je Gerät und Tag höchstens einer. */
	totalPings: number;
	versions: Record<string, number>;
	platforms: Record<string, number>;
}
...
/** Erzeugt den Datums-String "YYYY-MM-DD" für einen Zeitstempel. */
 ...
 * Pro Gerät und Tag existiert maximal eine Zeile in der Datenbank.
 */
export function recordTelemetryPing(
	...
	// Version bzw. eine bekannte Plattform aussieht, wird nicht übernommen -
	// sonst genügen ein paar hundert erfundene Werte, um die Seite unlesbar zu
	// machen.
...
/**
 * Geräte an genau diesem Tag.
 *
 * Eine eigene Abfrage und nicht aus dem Verlauf gelesen: der Verlauf reicht nur
 * `days` Tage zurück, und bei `days = 1` fällt gestern heraus.
 */
...
/** Eindeutige Geräte ab diesem Datum (einschliesslich). */
 ...
 * Zusammenfassung.
 *
 * WAU und MAU hängen NICHT an `days`, sondern an ihrem eigenen Zeitfenster.
 * Sonst stünde unter "30 Tage" die Zahl eines kürzeren Verlaufs, sobald jemand
 * weniger Tage abfragt.
 *
 * Gezählt wird in SQLite. Je Gerät und Tag gibt es genau eine Zeile
 * (Eindeutigkeits-Index), also ist `count(*)` je Tag schon die Zahl der Geräte.
 */
...
/**
 * Löscht alte Rohdaten aus `telemetry_pings`, die älter als `retentionDays` Tage sind.
 */
export function cleanupOldTelemetry(
	...
	// Falls Drizzle-Instanz übergeben
