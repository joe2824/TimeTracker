...
/**
 * Was der Passkey-Verwalter später anzeigt.
 *
 * Ein Konto aus der Desktop-Anwendung trägt keinen Namen - dort steht dann die
 * Kennung, und im Schlüsselbund liest man eine nackte UUID. Stattdessen der
 * Name der Anwendung und die Adresse, unter der sie läuft.
 *
 * Der Wert wird beim Anlegen in den Passkey geschrieben und ändert sich danach
 * nicht mehr, auch wenn das Konto später einen Namen bekommt.
 */
...
export async function registrationOptions(displayName: string, userId: string) {
	...
	return generateRegistrationOptions({
		...
		// Der Passkey soll auf dem Gerät bleiben und dort auffindbar sein - nur
...
export async function authenticationOptions() {
	return generateAuthenticationOptions({
		...
		// für diese Adresse hat. Eine Liste verriete ausserdem, welche Konten es
		// gibt.
...
export async function verifyAuthentication(
	...
	expectedChallenge: string,
	/** Ob der Authentifikator den Menschen geprüft haben muss. */
	requireUserVerification = false
) {
	...
	// Den Zähler mitführen: springt er zurück, ist der Authentifikator geklont.
	// Wir weisen das nicht ab (viele Passkeys zählen gar nicht), aber der Wert
	// gehört festgehalten.
...
/** Das Konto anlegen, zu dem der erste Passkey gehört. */
