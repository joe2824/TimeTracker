// Ein Passkey ohne Verpackung ist ein Vault ohne Schlüssel - und das fällt
// erst auf, wenn jemand sich auf einem neuen Gerät anmeldet. Deshalb entstehen
...
describe("storeWrap", () => {
	...
	it("je Passkey genau eine Verpackung", () => {
		db.transaction((tx) => storeCredential(tx, ANNA, cred("cred-1"), undefined));
	...
	it("eine Verpackung auf einen fremden oder erfundenen Passkey wird abgelehnt", () => {
		db.transaction((tx) => storeCredential(tx, BODO, cred("bodos-passkey"), undefined));

		expect(() =>
			db.transaction((tx) =>
				storeWrap(tx, ANNA, { kind: "passkey", payload: "p", credentialId: "bodos-passkey" })
			)
		).toThrow();

		expect(() =>
			db.transaction((tx) =>
				storeWrap(tx, ANNA, { kind: "passkey", payload: "p", credentialId: "nie-angelegt" })
			)
		).toThrow();
	});
