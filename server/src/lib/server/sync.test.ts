...
describe("Ablegen", () => {
	...
	it("weist eine Aenderung auf veraltetem Stand ab", () => {
		// Der Kern des Ganzen: zwei Geräte dürfen sich nicht gegenseitig
		// überschreiben, ohne die Änderung des anderen gesehen zu haben.
		pushRecords(db, ANNA, "g1", [rec("e1")]);
	...
	it("legt bei einer Loeschung einen Loeschmarker an, statt die Zeile zu entfernen", () => {
		// Ohne Löschmarker hält ein Gerät, das die Löschung verpasst hat, seinen
		// alten Stand für gültig und lädt ihn beim nächsten Mal wieder hoch.
		pushRecords(db, ANNA, "g1", [rec("e1")]);
	...
	it("prueft die Groesse VOR dem Schreiben, nicht mittendrin", () => {
		// Sonst läge der erste Datensatz schon da, wenn der zweite auffliegt.
		const huge = "x".repeat(64 * 1024 + 1);
		expect(() =>
...
describe("Abholen", () => {
	...
	it("eine leere Bucket-Liste liefert nichts - nicht alles", () => {
		// Der Unterschied ist der Punkt: eingeschränkt auf nichts ist nicht
		// dasselbe wie gar nicht eingeschränkt.
		pushRecords(db, ANNA, "g1", [rec("e1", { bucket: "juli" }), rec("s1", { bucket: null })]);
...
describe("Mandantentrennung", () => {
	...
	it("gleiche Ids in zwei Konten stoeren einander nicht", () => {
		// Die Ids sind zufällig, aber der Server darf sich nicht darauf verlassen.
	...
	it("Bodo kann Annas Datensatz nicht ueberschreiben", () => {
		...
		// Bodo schreibt mit derselben Id - für ihn ist sie neu.
...
describe("Zwei Geraete am selben Konto", () => {
	it("der Nachzuegler fuehrt zusammen und kommt beim zweiten Versuch durch", () => {
		// Der vollständige Ablauf, wie ihn der Client durchläuft.
		...
		// Er übernimmt den Serverstand und schickt auf dessen Fassung erneut.
	...
	it("das Geraet erkennt seine eigenen Aenderungen wieder", () => {
		// Damit ein Gerät nicht auf seinen eigenen Weckruf hin neu lädt.
