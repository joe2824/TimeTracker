...
describe("Token-Eimer", () => {
	...
	it("sagt, wie lange zu warten ist", () => {
		...
		// 60 je Minute heisst: eine Sekunde bis zum nächsten.
	...
	it("fuellt mit der Zeit wieder auf", () => {
		for (let i = 0; i < 5; i++) takeAttempt("a", RATE, 1000);
		expect(takeAttempt("a", RATE, 1000).allowed).toBe(false);
		// Zwei Sekunden später sind zwei Versuche nachgeflossen.
	...
	it("fuellt nie ueber den Rand", () => {
		...
		// Eine Stunde Ruhe - der Eimer ist voll, nicht übervoll.
		for (let i = 0; i < 5; i++) {
	...
	it("haelt die Schluessel auseinander", () => {
		for (let i = 0; i < 5; i++) takeAttempt("a", RATE, 1000);
		expect(takeAttempt("a", RATE, 1000).allowed).toBe(false);
		// Ein anderer Aufrufer ist davon unberührt - sonst wäre die Bremse
	...
	it("wirft nur volle Eimer weg", () => {
		...
		// Direkt danach aufräumen: beide sind zu jung, beide bleiben.
...
describe("Nachsehen ohne zu verbrauchen", () => {
	...
	it("verbraucht selbst nichts", () => {
		// Der Punkt: wer wartet, fragt oft nach. Dürfte das Nachsehen selbst
		// zählen, wäre die Bremse genau für den gedacht, den sie nicht treffen
		// soll.
