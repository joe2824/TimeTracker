...
/// Eine Zeile an die heutige Protokolldatei anhängen – im selben Format wie das
...
/// Abstürze des Rust-Teils mitschreiben.
...
/// damit die Beta-Liste ihn als Rückfall mitführen kann.
#[cfg(desktop)]
...
/// Manifest jeder Build überschreibt – auch ein stabiler. Wer Betas anhat,
/// bekommt damit immer den neuesten Stand und bleibt nicht auf einer Vorabversion
/// sitzen, nur weil längere Zeit keine neue Beta kam.
#[cfg(desktop)]
...
/// liest. Stabil bleibt als zweiter Eintrag der Rückfall.
#[cfg(desktop)]
...
pub struct TrayState {
    /// Name der laufenden Aktivität, falls ein Timer laeuft.
...
/// Das Flyout-Fenster – vorhandenes zurückgeben, sonst aus seinem Config-Block bauen.
...
fn tray_window(app: &tauri::AppHandle) -> Option<tauri::WebviewWindow> {
    // Ein Bauvorgang zur Zeit: zwei Anlässe dicht hintereinander (erster
...
fn tray_icon(running: bool) -> tauri::image::Image<'static> {
    // Groesse, in der das System das Icon zeichnet. Ändert sich nur mit der
    // Skalierung, also faktisch nie während des Betriebs.
    #[cfg(windows)]
    let px = {
...
/// Dasselbe für das Fenster – also für die Taskleiste.
#[cfg(desktop)]
...
///
/// Reicht für eine Handvoll Datensaetze ueber eine normale Leitung. Laenger
...
/// Flyout oeffnen wie ein Klick aufs Tray-Icon – für den Dev-Knopf in den
...
pub fn run() {
    ...
    // Tauris Tokio-Laufzeit für diesen Thread setzen, bevor irgendein Plugin
    // aufgebaut wird.
    #[cfg(desktop)]
