...
/// Was aus dem Schutz herauskam - und ob er überhaupt griff.
#[derive(Serialize)]
pub struct Protected {
    /// Base64 des geschützten Werts (oder des Klartexts, wenn `protected` false ist).
    pub data: String,
    /// Ob das Betriebssystem den Wert wirklich geschützt hat.
    pub protected: bool,
}
...
/// Einen Wert schützen. Gibt zurück, OB er geschützt wurde.
///
/// Das Feld `protected` ist kein Beiwerk: die Oberfläche sagt dem Nutzer
...
pub fn protect_secret(mut plain: String) -> Protected {
    ...
    // Die Kopie, die über die Brücke kam, wird hier nicht mehr gebraucht.
    // Ohne das bliebe sie bis zur nächsten Wiederverwendung im Speicher stehen.
    plain.zeroize();
    sealed_value
}

/// Einen geschützten Wert wieder öffnen.
///
